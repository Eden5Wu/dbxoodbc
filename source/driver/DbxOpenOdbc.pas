...
begin
  ...
  begin
    if OutLength > 0 then
      System.AnsiStrings.StrCopy( OutPropValue, PAnsiChar(sValue));
  end
  else
  begin
    inc(OutLength);
...
var
  ...
  function SQLGetDiagRecLevel2(HandleType: SqlSmallint;
    ...
    begin
      ...
      begin
        inc(iSQLGetDiagRec2);
        System.AnsiStrings.StrLCopy(MessageText, PAnsiChar(AnsiString(e.Message)), BufferLength);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        fOdbcErrorLines.Add('');
        fOdbcErrorLines.Add(string('ODBC SqlState:        ' + System.AnsiStrings.StrPas(SqlState)));
      end
      else
      if bClearErrorCount > 0 then
      begin
        New(vPString);
        fNewErrorLines.Add(vPString);
        vPString^ := #13#10#13#10 + 'ODBC SqlState:        ' + System.AnsiStrings.StrPas(SqlState);
      end;
      ...
      CheckCodeText := System.AnsiStrings.StrPas(pMessageText);
    ...
    case xeDOption of
      ...
      xeDrvVersion:
        begin
          ...
          begin
            System.AnsiStrings.StrLCopy(PAnsiChar(PropValue), PAnsiChar(cDbxDriverVersionString), MaxLength);
      ...
      xeDrvProductVersion:
        begin
          ...
          if fDBXVersion >= 30 then
          begin
            System.AnsiStrings.StrLCopy(PAnsiChar(PropValue), PAnsiChar(cDbxProductVersionString), MaxLength);
...
begin
  ...
  try
    ...
    begin
      DriverList.Add(string(System.AnsiStrings.StrPas(sDriverDescBuffer)));
      sDriverDescBuffer[0] := cNullAnsiChar;
      sDriverAttributesBuffer[0] := cNullAnsiChar;
...
{$IFNDEF _RegExprParser_}
  ...
  var
    ...
    begin
      ...
      T_start := 0;
      T_end := System.AnsiStrings.StrLen(TableName) - 1;
      ...
      begin
        ...
        System.AnsiStrings.StrLCopy(PansiChar(sCatalogName), @TableName[C_start], C_end - C_start + 1);
      end;
      if (S_end <> 0) then
      begin
        ...
        System.AnsiStrings.StrLCopy(PAnsiChar(sSchemaName), @TableName[S_start], S_end - S_start + 1);
      end;
      ...
      System.AnsiStrings.StrLCopy(PAnsiChar(sObjectName), @TableName[T_start], T_end - T_start + 1);
    end;
    // ---
    ...
    begin
      ...
      vTable := {$IFDEF _D12UP_}AnsiStrings.{$ENDIF}
        StringReplace(System.AnsiStrings.StrPas(TableName), '::', ':', [rfReplaceAll]);
      ...
      begin
        ...
        begin
          ...
          System.AnsiStrings.StrLCopy(PansiChar(sCatalogName), PAnsiChar(vStr), Length(vStr));
        end;
      ...
      begin
        ...
        begin
          ...
          System.AnsiStrings.StrLCopy(PAnsiChar(sSchemaName), PAnsiChar(vStr), Length(vStr));
        end;
        vTable := Copy(vTable, p + 1, Length(vTable) - p);
        if Length(vTable) = 0 then
        begin
      ...
      System.AnsiStrings.StrLCopy(PAnsiChar(sObjectName), PAnsiChar(vTable), Length(vTable));
    end;
    // ---
  begin
  ...
  begin
    ...
    begin
      ...
      if (aCurrentCatalogLen >= 0) and (aCurrentCatalogLen <= fOdbcMaxCatalogNameLen) then
        ...
        Result := System.AnsiStrings.StrPas( PAnsiChar(Result) );
    end;
  end;
  {$IFDEF _TRACE_CALLS_}
...
begin
  ...
  {$IFDEF _TRACE_CALLS_}try try {$R+} LogEnterProc('TSqlConnectionOdbc.connect', ['ConnectString =', fConnConnectionString,
    'ServerName=', System.AnsiStrings.StrPas(ServerName), 'UserName =', System.AnsiStrings.StrPas(UserName){, 'Password =', System.AnsiStrings.StrPas(Password)}]); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF}
  {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBXCB_}
  if Assigned(fDbxTraceCallbackEven) then
    DbxCallBackSendMsgFmt(cTDBXTraceFlags_Connect, 'ISqlConnection.Connect: conn: "$%x" , user "%s", server "%s"', [Integer(Self), System.AnsiStrings.StrPas(UserName), System.AnsiStrings.StrPas(ServerName)]);
  {$ENDIF}
  if fConnected then
    Exit;
  ...
  try
    ...
    if bIsConnConnectionString then
      ...
      fOdbcConnectString := System.AnsiStrings.StrPas(ServerName);
    ...
    begin
      ...
      begin
        fDbxCatalog := Trim(System.AnsiStrings.StrPas(ServerName));
        if fDbxCatalog = '?' then
          fDbxCatalog := '';
    ...
    sUserName := System.AnsiStrings.StrPas(UserName);
    sPassword := System.AnsiStrings.StrPas(Password);
...
//todo: SQLDriverConnectW
    ...
    begin
      ...
      begin
        ...
        fOdbcReturnedConnectString := System.AnsiStrings.StrPas(PAnsiChar(fOdbcReturnedConnectString));
    ...
    *)
    ...
    if (Self.fOdbcDriverType = eOdbcDriverTypeInformix)
        and (System.AnsiStrings.StrLComp( PAnsiChar(UpperCase(fOdbcDriverName)), 'ICLI', 4) = 0) // (It is meaningful only for the native informix driver)
    then
    begin
...
{$IFDEF _RegExprParser_}
    ...
    if Assigned(fDbxTraceCallbackEven) then
      DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISqlConnection.Connect (object name parser): conn: "$%x": "%s"; QuoteChar: <%s>',
        [Integer(Self), vObjectNameTemplateInfo.sName, ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(@fQuoteChar)))]);
    {$ENDIF}
    CreateRegExpObjectNameParser(vObjectNameTemplateInfo, System.AnsiStrings.StrPas(PAnsiChar(@fQuoteChar)));
  {$IFDEF _TRACE_CALLS_}
...
{$IFDEF _TRACE_CALLS_}
  ...
  Inc(Len);
  LogInfoProc([AnsiString(Format('%2d', [Len]))+'-QuoteChar =', '<'+System.AnsiStrings.StrPas(PAnsiChar(@fQuoteChar))+'>']);
  Inc(Len);
...
begin
  ...
  try
    ...
    case fDbmsType of
      eDbmsTypeOracle:
        begin
          if ((System.AnsiStrings.StrLComp(pBuffer, 'SQORA', 5) = 0)) // Oracle ODBC Driver
          { Microsoft ODBC for Oracle always demand '.' }
            {- or (System.AnsiStrings.StrLComp(pBuffer, 'MSORCL', 6) = 0)   // Microsoft ODBC for Oracle}
          {{???: or (System.AnsiStrings.StrLComp(pBuffer, 'IVORA', 5) = 0)    // DataDirect Oracle Wire Protocol ODBC Driver}
          {{???: or (System.AnsiStrings.StrLComp(pBuffer, 'IVOR', 4) = 0)     // DataDirect Oracle ODBC Driver}
          {{???: or (System.AnsiStrings.StrLComp(pBuffer, 'PBOR', 4) = 0)     // PB INTERSOLV OEM ODBC Driver}
          then
          begin
...
begin
  ...
  if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
    fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLGetInfo(SQL_DBMS_NAME)',
      SQL_HANDLE_DBC, fhCon, nil, Self);
  fDbmsName := System.AnsiStrings.StrPas(PAnsiChar(sBuffer));
  ...
  begin
    ...
    {$ENDIF}
      ...
      // IBM DB2/400 SQL:
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'DB2', 3)=0) then
  begin
    ...
    {$ENDIF}
      ...
      // My SQL:
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'MYSQL', 5)=0) then
  begin
  ...
  begin
    ...
    {$ENDIF}
      ...
      // DBASE II, IV, V
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'DBASE', 5)=0) then // DBASE II, IV, V
  begin
  ...
  begin
    ...
    {$ENDIF}
      ...
      // ORACLE
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'ORACLE', 6)=0) then
  begin
  ...
  begin
    ...
    {$ENDIF}
      ...
      // FIREBIRD
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'FIREBIRD', 8)=0) then // 'FIREBIRD / INTERBASE(R)'
  begin
    fDbmsType := eDbmsTypeInterbase;
    {$IFDEF _DBXCB_}
    S := 'eDbmsTypeInterbase';
  ...
  begin
    ...
    {$ENDIF}
      ...
      // THINK SQL
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'THINKSQL', 8)=0) then // 'THINKSQL RELATIONAL DATABASE MANAGEMENT SYSTEM'
  begin
  ...
  begin
    ...
    {$ENDIF}
      ...
      // POSTGRE SQL
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'POSTGRESQL', 10)=0) then
  begin
  ...
  begin
    ...
    {$ENDIF}
      ...
      // Flash Filler
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'TURBOPOWER FLASHFILER', 21)=0) then
  begin
    ...
    {$ENDIF}
      ...
      // Oterro
  else if (System.AnsiStrings.StrLComp(PAnsiChar(uDbmsName), 'OTERRO', 6)=0) then
  begin
  ...
  begin
    fDbmsVersionString := System.AnsiStrings.StrPas(PAnsiChar(sBuffer));
  ...
  if OdbcRetcode <> OdbcApi.SQL_SUCCESS then
    fOwnerDbxDriver.OdbcCheck(OdbcRetcode, 'SQLGetInfo(SQL_DRIVER_ODBC_VER)',
      SQL_HANDLE_DBC, fhCon, nil, Self);
  fOdbcDriverVersionString := System.AnsiStrings.StrPas(PAnsiChar(sBuffer));
  ...
  begin
    fOdbcDriverName := System.AnsiStrings.StrPas(PAnsiChar(sBuffer));
  ...
  {$ENDIF}
    // SQL Base:
  if (System.AnsiStrings.StrLComp(pBuffer, 'C2GUP', 5) = 0) or
    (System.AnsiStrings.StrLComp(pBuffer, 'IVGUP', 5) = 0) // DataDirect SQLBase ODBC Driver
    or
    (System.AnsiStrings.StrLComp(pBuffer, 'PBGUP', 5) = 0) // INTERSOLV OEM SQLBase ODBC Driver
  then
  begin
    ...
    // SQL Server 2005 Up:
  //
  //else if (System.AnsiStrings.StrLComp(pBuffer, ?, 7) = 0) then // SQL Server 2005
  //  fOdbcDriverType := eOdbcDriverTypeMsSqlServer2005Up
  //
  end
    // SQL Server:
  else if (fDbmsType = eDbmsTypeMsSqlServer) and (
    (System.AnsiStrings.StrLComp(pBuffer, 'SQLSRV', 6) = 0)    // SQL Server ( Microsoft ODBC Driver )
    or (System.AnsiStrings.StrLComp(pBuffer, 'SQLNCLI', 7) = 0)    // SQL Native Client  ( Microsoft ODBC Driver )
    or (System.AnsiStrings.StrLComp(pBuffer, 'IVSS', 4) = 0)   // DataDirect SQL Server ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'IVMSSS', 6) = 0) // DataDirect SQL Server Wire Protocol ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'PBSS', 4) = 0)   // PB INTERSOLV OEM SqlServer ODBC Driver
    or // extended comparing
    ((System.AnsiStrings.StrLComp(pBuffer, 'NTL', 3) = 0) and (pBuffer[5] = 'M'))
  ...
  begin
    ...
    // IBM DB2:
  else if (System.AnsiStrings.StrLComp(pBuffer, 'DB2CLI', 6) = 0) // IBM DB2 ODBC DRIVER
    or (System.AnsiStrings.StrLComp(pBuffer, 'LIBDB2', 6) = 0)    // IBM
    or (System.AnsiStrings.StrLComp(pBuffer, 'IVDB2', 5) = 0)     // DataDirect DB2 Wire Protocol ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'PBDB2', 5) = 0)     // INTERSOLV OEM ODBC Driver}
  then
  begin
    ...
    // IBM DB2/400 SQL:
  else if (System.AnsiStrings.StrLComp(pBuffer, 'CWBODBC', 7) = 0)   // IBM DB2/400 SQL
  then
  begin
  ...
  else if System.AnsiStrings.StrLComp(pBuffer, 'ODBCJT', 6) = 0
...
// My SQL ODBC Version 3 Driver:
  else if System.AnsiStrings.StrLComp(pBuffer, 'MYODBC3', 7) = 0 then
    fOdbcDriverType := eOdbcDriverTypeMySql3
    // My SQL:
  else if System.AnsiStrings.StrLComp(pBuffer, 'MYODBC', 6) = 0 then
  begin
    ...
    // INFORMIX:
  else if (System.AnsiStrings.StrLComp(pBuffer, 'ICLI', 4) = 0) // "INFORMIX 3.32 32 BIT" ODBC Driver
    // begin (other informix linux drivers):
  or (System.AnsiStrings.StrLComp(pBuffer, 'LIBTHCLI', 8) = 0)
  or (System.AnsiStrings.StrLComp(pBuffer, 'LIBIFCLI', 8) = 0)
  or (System.AnsiStrings.StrLComp(pBuffer, 'LIBIFDRM', 8) = 0)
  or (System.AnsiStrings.StrLComp(pBuffer, 'IDMRM', 5) = 0)
    // end.
  or (System.AnsiStrings.StrLComp(pBuffer, 'IVINF', 5) = 0)  // DataDirect Informix ODBC Driver
  or (System.AnsiStrings.StrLComp(pBuffer, 'IVIFCL', 6) = 0) // DataDirect Informix Wire Protocol ODBC Driver
  or (System.AnsiStrings.StrLComp(pBuffer, 'PDINF', 5) = 0)  // INTERSOLV Inc ODBC Driver (1997. Now is DataDirect)
  or (System.AnsiStrings.StrLComp(pBuffer, 'PBINF', 5) = 0)  // PB INTERSOLV OEM ODBC Driver
  or // extended comparing
  ((System.AnsiStrings.StrLComp(pBuffer, 'NTL', 3) = 0) and (pBuffer[5] = 'I'))
  ...
  begin
    ...
    (System.AnsiStrings.StrLComp(pBuffer, 'SYODASE', 7) = 0)       // SYBASE ACE ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'DBODBC', 6) = 0)     // Adaptive Server Anywhere
    or (System.AnsiStrings.StrLComp(pBuffer, 'SYSYBNT', 7) = 0) )  // Sybase System 11
  then
  begin
  ...
  else if ( System.AnsiStrings.StrLComp(pBuffer, 'IVASE', 5) = 0 ) then // DataDirect SybaseWire Protocol ODBC Driver
  begin
    fOdbcDriverType := eOdbcDriverTypeSybase;
    {$IFDEF _DBXCB_}
    S := 'eOdbcDriverTypeSybase';
  ...
  (*
    ...
    (System.AnsiStrings.StrLComp(pBuffer, 'SYSYBNT', 7) = 0)       // Sybase System 11
  ) then
  begin
    fDbmsType := eDbmsTypeSybase;
    fOdbcDriverType := eOdbcDriverTypeSybase;
  ...
  *)
    // SQLite:
  //else if (System.AnsiStrings.StrLComp(pBuffer, 'SQLITEODBC', 10) = 0)
  //  or (System.AnsiStrings.StrLComp(pBuffer, 'SQLITE3ODBC', 11) = 0) then
  else if System.AnsiStrings.StrLComp(pBuffer, 'SQLITE', 6) = 0  then
  begin
    ...
    // INTERBASE:
  else if System.AnsiStrings.StrLComp(pBuffer, 'IB6ODBC', 7) = 0 {// Easysoft ODBC Driver} then
  begin
  ...
  else if System.AnsiStrings.StrLComp(pBuffer, 'ODBCJDBC', 8) = 0 then {// IBPhoenix ODBC Driver: http://www.ibphoenix.com/}
  begin
    fOdbcDriverType := eOdbcDriverTypeInterbase;
    {$IFDEF _DBXCB_}
    S := 'eOdbcDriverTypeInterbase';
  ...
  else if System.AnsiStrings.StrLComp(pBuffer, 'IB6XTG', 6) = 0 then
  ...
  else if System.AnsiStrings.StrLComp(pBuffer, 'IBGEM', 5) = 0 then
  begin  {// Gemini ODBC: http://www.ibdatabase.com/ }
    ...
    // Think SQL:
  else if System.AnsiStrings.StrLComp(pBuffer, 'THINKSQL', 8) = 0 {// ThinkSQL ODBC Driver} then
  begin
    ...
    // ORACLE:
  else if (System.AnsiStrings.StrLComp(pBuffer, 'SQORA', 5) = 0) // Oracle ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'MSORCL', 6) = 0)   // Microsoft ODBC for Oracle
    or (System.AnsiStrings.StrLComp(pBuffer, 'IVORA', 5) = 0)    // DataDirect Oracle Wire Protocol ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'IVOR', 4) = 0)     // DataDirect Oracle ODBC Driver
    or (System.AnsiStrings.StrLComp(pBuffer, 'PBOR', 4) = 0)     // PB INTERSOLV OEM ODBC Driver
  ...
  else if (System.AnsiStrings.StrLComp(pBuffer, 'INOLE', 5) = 0) {// MERANT ODBC-OLE DB Adapter Driver} then
  begin
    ...
    // Pervasive.SQL
  else if (System.AnsiStrings.StrLComp(pBuffer, 'W3ODBCCI', 8) = 0) {// Pervasive.SQL ODBC Driver Client Interface } then
  begin
  ...
  else if (System.AnsiStrings.StrLComp(pBuffer, 'W3ODBCEI', 8) = 0) {// Pervasive.SQL ODBC Driver Engine Interface } then
  begin
    fOdbcDriverType := eOdbcDriverTypePervasive;
    {$IFDEF _DBXCB_}
    S := 'eOdbcDriverTypePervasive';
    ...
    // FlasfFiller
  else if (System.AnsiStrings.StrLComp(pBuffer, 'NXODBCDRIVER', 12) = 0) {// NexusDb FlashFiler Driver } then
  begin
    ...
    // PostgreSQL
  else if (System.AnsiStrings.StrLComp(pBuffer, 'PSQLODBC', 8) = 0) then
  begin
    ...
    // Cache
  else if (System.AnsiStrings.StrLComp(pBuffer, 'CACHEODBC', 9) = 0)  then
  begin
    ...
    (System.AnsiStrings.StrLComp(pBuffer, 'IVDBF', 5) = 0)  {MERANT dBASE File ODBC DRIVER}
    or
    (System.AnsiStrings.StrLComp(pBuffer, 'PBDBF', 5) = 0)  {PB INTERSOLV OEM dBASE File ODBC DRIVER}
  then
  begin
    ...
    (System.AnsiStrings.StrLComp(pBuffer, 'SQLOD', 5) = 0)  then {'SAP DB' ODBC Driver by SAP AG}
  begin
   ...
   // PARADOX
      ...
      (System.AnsiStrings.StrLComp(pBuffer, 'PBIDP', 5) = 0) // PB INTERSOLV OEM ParadoxFile ODBC Driver:
      or                                 //   supports Paradox 3.0, 3.5, 4.0, 4.5, 5.0, 7.0, and 8.0 tables.
      (System.AnsiStrings.StrLComp(pBuffer, 'IVDP', 5) = 0)  // DataDirect Paradox File (*.db) ODBC Driver:
    ) then
  begin
    ...
    {$ENDIF}
    //if (System.AnsiStrings.StrLComp(Buffer, 'PBIDP', 5) = 0) then // ???: 'IVDP'
    begin
      if fConnectionOptions[coFldReadOnly] = osDefault then
        fConnectionOptions[coFldReadOnly] := osOff;
      if fConnectionOptions[coParamDateByOdbcLevel3] = osDefault then
   ...
   // Btrieve
    ...
    (System.AnsiStrings.StrLComp(pBuffer, 'IVBTR', 5) = 0 ) // DataDirect Btrieve (*.dta) ODBC Driver
    or
    (System.AnsiStrings.StrLComp(pBuffer, 'PBBTR', 5) = 0)  // PB INTERSOLV OEM Btrieve ODBC Driver:
    ) then
  begin
   ...
   // OpenIngres
      ...
      (System.AnsiStrings.StrLComp(pBuffer, 'PBOING', 6) = 0) // PB INTERSOLV OEM OpenIngres ODBC Driver:
      or
      (System.AnsiStrings.StrLComp(pBuffer, 'PBOI2', 5) = 0)  // PB INTERSOLV OEM OpenIngres2 ODBC Driver:
    ) then
  begin
    ...
    // FoxPro
      ...
      (System.AnsiStrings.StrLComp(pBuffer, 'VFPODBC', 7) = 0) // Microsoft Visual FoxPro Driver (*.dbf)    'VFPODBC'
      or
      (System.AnsiStrings.StrLComp(pBuffer, 'IVDBF', 5) = 0) // DataDirect FoxPro 3.0 database (*.dbc)    'IVDBF'
    ) then
  begin
    ...
    {$ENDIF}
    if (System.AnsiStrings.StrLComp(pBuffer, 'IVDBF', 5) = 0) then  // ???: 'IVDBF'
    begin
      if fConnectionOptions[coFldReadOnly] = osDefault then
        fConnectionOptions[coFldReadOnly] := osOff;
      if fConnectionOptions[coParamDateByOdbcLevel3] = osDefault then
    ...
    // Progress
      ...
      ( System.AnsiStrings.StrLComp(pBuffer, 'IVPRO', 5) = 0 )  // DataDirect Progress ODBC Driver
      or
      (System.AnsiStrings.StrLComp(pBuffer, 'PBPRO', 5) = 0)    // INTERSOLV OEM Progress ODBC Driver
      or
      (System.AnsiStrings.StrLComp(pBuffer, 'PGOE', 4) = 0)     // [DataDirect][ODBC OPENEDGE driver]
    ) then
  begin
    ...
    // Oterro RBase 2, 3
      ...
      ( System.AnsiStrings.StrLComp(pBuffer, 'OT2K_32', 7) = 0 )
      or
      (System.AnsiStrings.StrLComp(pBuffer, 'OTERRO', 6) = 0)
    ) then
  begin
...
//#0xx: todo: need check for edit table
  ...
  case fOdbcDriverType of
    ...
    eOdbcDriverTypeMsSqlServer:
      begin
        ...
        begin
          ...
          // unknown catalog)
          if (System.AnsiStrings.StrLComp(pBuffer, 'IVSS', 4) = 0) then
          begin
             fConnectionOptionsDrv[coSupportsCatalog] := osOff;
    ...
    eOdbcDriverTypeInformix:
      begin
        fWantQuotedTableName := False;
        //if ( System.AnsiStrings.StrLComp(pBuffer, 'PDINF', 5) = 0 ) // INTERSOLV Inc ODBC Driver (1997. Now is DataDirect)
        fConnectionOptionsDrv[coSupportsCatalog] := osOff; // INFORMIX supports operation with
        // the catalog, but usage of this option is inconvenient for the developers and there is no
        // large sense  by work with INFORMIX. If you want to work with the catalog, comment out this line.
        fConnectionOptionsDrv[coIgnoreUnknownFieldType] := osOn;
...
begin
  if Error <> nil then
    System.AnsiStrings.StrCopy(Error, PAnsiChar(AnsiString(fConnectionErrorLines.Text)));
  fConnectionErrorLines.Clear;
  Result := DBXERR_NONE;
end;
        ...
        begin
          ...
          if Assigned(fDbxTraceCallbackEven) then
            DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISqlConnection.GetMetaDataOption MetaDatabaseName: (%s)', [
              ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(PropValue))) ]);
          {$ENDIF}
        end
        else
          Result := DBXERR_INVALIDPARAM;
...
{.$ENDIF} // of: $IFDEF _K3UP_}
  ...
  begin
     ...
     if (ConnectAttrLength >= 0) and (ConnectAttrLength <= fOdbcMaxCatalogNameLen) then
       ...
       S := System.AnsiStrings.StrPas( PAnsiChar(S) );
    {$IFDEF _DBXCB_}
    if Assigned(fDbxTraceCallbackEven) then
  ...
  begin
    case xeDOption of
      ...
      xeConnConnectionName:
        Result := AnsiString(System.AnsiStrings.StrPas(PAnsiChar(PropValue)));
    ...
    {.$IFDEF _D7UP_}
      ...
      xeConnCatalogName:
        ...
        begin
          ...
          {$IFDEF _DBXCB_}
          if Assigned(fDbxTraceCallbackEven) then
            DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISqlConnection.GetOption ConnCatalogName: "%s"', [ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(PropValue)))]);
          {$ENDIF}
        end
        else
          Result := DBXERR_INVALIDPARAM;
      xeConnSchemaName:
        ...
        begin
          ...
          {$IFDEF _DBXCB_}
          if Assigned(fDbxTraceCallbackEven) then
            DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISqlConnection.GetOption ConnSchemaName: "%s"', [ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(PropValue)))]);
          {$ENDIF}
        end
        else
          Result := DBXERR_INVALIDPARAM;
      ...
      xeConnQuotedObjectName:
        ...
        begin
          ...
          {$IFDEF _DBXCB_}
          if Assigned(fDbxTraceCallbackEven) then
            DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISqlConnection.GetOption ConnQuotedObjectName: %s', [ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(PropValue)))]);
          {$ENDIF}
        end
        else
          Result := DBXERR_INVALIDPARAM;
...
var
  ...
  begin
    if (fOwnerDbxDriver.fDBXVersion < 30) and  (fOwnerDbxDriver.fClientVersion >= 30)
      ...
      Result := AnsiString(System.AnsiStrings.StrPas(PAnsiChar(lValue)));
  end;
  // ---
  {.$IFDEF _K3UP_}
...
var
  ...
  begin
    case xeConnectOption of
      xeConnRoleName:
        Result := System.AnsiStrings.StrPas(PAnsiChar(lValue));
      xeConnTxnIsoLevel:
...
begin
  ...
  try
    ...
    {.$IFDEF _D7UP_}
      xeConnQualifiedName:
        begin
          fQualifiedName := System.AnsiStrings.StrPas(PAnsiChar(lValue));
          {$IFDEF _DBXCB_}
          if Assigned(fDbxTraceCallbackEven) then
      ...
      xeConnCatalogName:
        ...
        begin
          ...
          S := System.AnsiStrings.StrPas(PAnsiChar(lValue));
    ...
    {.$ENDIF} //of: IFDEF _D7UP_
      xeConnConnectionString:
        if not fConnected then
        begin
          fConnConnectionString := System.AnsiStrings.StrPas(PAnsiChar(lValue));
          {$IFDEF _DBXCB_}
          if Assigned(fDbxTraceCallbackEven) then
...
//        Result := DoMetadataQueryEmpty;
  ...
  begin
    if bUseUnicodeOdbc then
      ...
      sDbxCmdText := WideString(System.AnsiStrings.StrPas(SQL));
...
// todo: if fChkDbxSQLParamDelim then
    ...
    begin
      fSql := System.AnsiStrings.StrPas(SQL);
// todo: if fChkDbxSQLParamDelim then
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try {$R+} LogEnterProc('TSqlCommandOdbc.ExecuteImmediate', ['SQL =', System.AnsiStrings.StrPas(SQL)]); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF} {$ENDIF _TRACE_CALLS_}
  {$IFDEF _DBXCB_}
  if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
    fOwnerDbxConnection.DbxCallBackSendMsg(cTDBXTraceFlags_Execute, 'ISQLCommand.ExecuteImmediate: ' + AnsiString(System.AnsiStrings.StrPas(SQL)));
  {$ENDIF}
...
begin
  if Error <> nil then
    System.AnsiStrings.StrCopy(Error, PAnsiChar(AnsiString(fOwnerDbxConnection.fConnectionErrorLines.Text)));
      ...
      xeCommCursorName:
        ...
        begin
          ...
          if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) and (MaxLength > 0) and Assigned(PropValue) then
            fOwnerDbxConnection.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISQLCommand.GetOption CommCursorName: %s', [ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(PropValue)))]);
          {$ENDIF}
        end
        else
          Result := DBXERR_INVALIDPARAM;
...
//todo: ????? not tested it:
        ...
        fldFMTBCD:
          { // OLD:
          begin
            SetString(s, fValue.OdbcParamValueString, System.AnsiStrings.StrLen(fValue.OdbcParamValueString));
            PBcd(Value)^ := StrToBcd(s);
          end; // }
          Str2BCD(fValue.OdbcParamValueString,
            System.AnsiStrings.StrLen(fValue.OdbcParamValueString), PBcd(Value)^, cDecimalSeparatorDefault);
        fldBOOL:
...
begin
  ...
  {$IFDEF _DBXCB_}
  if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
    fOwnerDbxConnection.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Prepare, 'ISQLCommand.Prepare ParamCount: %d; SQL: %s', [ParamCount, System.AnsiStrings.StrPas(SQL)]);
  {$ENDIF}
  if SQL = nil then
  begin
    Result := DBXERR_INVALIDPARAM;
...
var
  ...
  begin
    case xeSqlCommandOption of
      ...
      xeCommCursorName:
        Result := AnsiString(System.AnsiStrings.StrPas(PAnsiChar(ulValue)));
      xeCommStoredProc:
        Result := AnsiString(BoolToStr(Boolean(ulValue)));
      xeCommPackageName:
        Result := AnsiString(System.AnsiStrings.StrPas(PAnsiChar(ulValue)));
    ...
    case xeSqlCommandOption of
      ...
      xeCommCursorName:
        begin
          ...
          {$IFDEF _DBXCB_}
          if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
            fOwnerDbxConnection.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Vendor, 'ISQLCommand.SetOption CommCursorName: %s', [ArgStrNull(System.AnsiStrings.StrPas(PAnsiChar(ulValue)))]);
          {$ENDIF}
        end;
      xeCommStoredProc:
        begin
...
{.$IFDEF _D7UP_}
      ...
      xeCommSchemaName:
        begin
          fSchemaName := System.AnsiStrings.StrPas(PAnsiChar(ulValue));
          {$IFDEF _DBXCB_}
          if Assigned(fOwnerDbxConnection.fDbxTraceCallbackEven) then
...
var
  ...
  begin
    ...
    begin
      sUTF8Buffer := AnsiToUtf8(string(System.AnsiStrings.StrPas(PAnsiChar(pBuffer))));
    end
    else
    begin
              ...
              begin
                ...
                begin
                  ...
                  if bUnicodeString then
                    ...
                    iPrecision := System.AnsiStrings.StrLen(PAnsiChar(pBuffer));
                  if iLength < LongWord(iPrecision) then
                    iLength := iPrecision;
                end
                ...
                begin
                  ...
                  if bUnicodeString then
                    ...
                    iPrecision := min(iPrecision, System.AnsiStrings.StrLen(PAnsiChar(pBuffer)));
                end;
              end;
            end
        ...
        fldFMTBCD:
          begin
            ...
            begin
              ...
              System.AnsiStrings.StrCopy((fValue.OdbcParamValueString, PAnsiChar(S)); }
            end
            else
            begin
              bIsNull := 1;
...
begin
  ...
  System.AnsiStrings.StrCopy(Error, PAnsiChar(AnsiString(fMetaDataErrorLines.Text)));
...
begin
  {$IFDEF _TRACE_CALLS_} Result := DBXERR_NONE; try try {$R+} LogEnterProc('TSQLMetaDataOdbc.getTables',
    ['TableName =', System.AnsiStrings.StrPas(TableName), 'TableType =', TableType]); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF}
  {$ENDIF _TRACE_CALLS_}
...
begin
  ...
  try
    case xeDOption of
      xeMetaCatalogName:
        fMetaCatalogName := ExtractCatalog(System.AnsiStrings.StrPas(PAnsiChar(PropValue)),
          fOwnerDbxConnection.fOdbcCatalogPrefix);
      xeMetaSchemaName:
        if (fOwnerDbxConnection.fConnectionOptions[coSupportsSchemaFilter] = osOn) then
          fMetaSchemaName := System.AnsiStrings.StrPas(PAnsiChar(PropValue));
...
{.$IFDEF _K3UP_}
      xeMetaPackageName:
        FMetaPackName := System.AnsiStrings.StrPas(PAnsiChar(PropValue));
{.$ENDIF}
    end;
  except
    on E: EDbxNotSupported do
...
{$IFDEF _D9UP_}{$ENDREGION}{$ENDIF}
                ...
                if (fOwnerDbxConnection.fOdbcDriverType = eOdbcDriverTypeInterbase) and
                   ( System.AnsiStrings.StrLComp(PAnsiChar(UpperCase(fOwnerDbxConnection.fOdbcDriverName)),
                     'IB6XTG', 6 ) = 0 )
                then
  ...
  begin
    ...
    if IsBlank then
      ...
      Str2BCD(fOdbcHostVarAddress.ptrAnsiChar,
        System.AnsiStrings.StrLen(fOdbcHostVarAddress.ptrAnsiChar), PBcd(Value)^, cDecimalSeparatorDefault);
    {$IFDEF _D9UP_}{$REGION 'COMMENTS'}{$ENDIF}
    (* // OLD:
    begin
      // with PBcd(Value)^ do
        ...
        begin
          ...
          if (d > cMaxBcdCharDigits) then
            raise EDbxOdbcError.Create(
              ...
              '"'+System.AnsiStrings.StrPas(fOdbcHostVarAddress.ptrAnsiChar) + '"'
            );
          {}
  ...
  try
    ...
    begin
      ...
      if IsBlank then
        ...
        iLength := System.AnsiStrings.StrLen(fOdbcHostVarAddress.ptrAnsiChar) + 1;
...
begin
  ...
  System.AnsiStrings.StrCopy(Error, PAnsiChar(AnsiString(fOwnerCommand.fOwnerDbxConnection.fConnectionErrorLines.Text)));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if bNotUnicodeString then
        begin  // debug: System.AnsiStrings.StrLen(aOdbcBindCol.fOdbcHostVarAddress.ptrAnsiChar)
          Move(fOdbcHostVarAddress.ptrAnsiChar^, pDestA^, vColValueSize);
...
begin
  ...
  fSqlConnectionOdbc.DecodeObjectFullName(
    System.AnsiStrings.StrPas(TableName), CatalogName, SchemaName, ObjectName);
...
begin
  ...
  System.AnsiStrings.StrCopy(Error, PAnsiChar(AnsiString(fSqlCursorErrorMsg.Text)));
...
begin
  {$IFDEF _TRACE_CALLS_}try try {$R+} LogEnterProc('TMetaTable.Create', ['Cat=', Cat, 'Schema =', Schema,
    'TableName =', TableName, 'TableType =', TableType, 'Remarks =', System.AnsiStrings.StrPas(Remarks)]); {$IFDEF RANGECHECKS_OFF}{$R-}{$ENDIF}
  {$ENDIF _TRACE_CALLS_}
  aCatLen := 0;
  ...
  if WantCatalog and Assigned(Cat) then
    vCatalogName := System.AnsiStrings.StrPas(Cat)
  else
    SetLength(vCatalogName, 0);

  if WantSchema and Assigned(Schema) then
    vSchemaName := System.AnsiStrings.StrPas(Schema)
  else
    SetLength(vSchemaName, 0);

  if Assigned(TableName) then
    vObjectName := System.AnsiStrings.StrPas(TableName)
  else
  ...
  fRemarks := System.AnsiStrings.StrPas(Remarks);
...
begin
  ...
  if Assigned(fSqlConnectionOdbc.fDbxTraceCallbackEven) then
    fSqlConnectionOdbc.DbxCallBackSendMsgFmt(cTDBXTraceFlags_MetaData,
      ...
      [System.AnsiStrings.StrPas(SearchCat), System.AnsiStrings.StrPas(SearchSchema), System.AnsiStrings.StrPas(SearchTableName), MakeTableTypeString(True) ]);
  {$ENDIF}

  if (not fSqlConnectionOdbc.fSupportsCatalog) or (Assigned(SearchCat)
    and ( (SearchCat^ = cNullAnsiChar) or (System.AnsiStrings.StrComp(SearchCat, '?') = 0)) ) then
  begin
    SearchCat := nil;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        if bUnicodeApi then
        begin
          wsSchema := WideString(System.AnsiStrings.StrPas(SearchSchema));
      ...
      begin
        if bUnicodeApi then
        begin
          wsCat := WideString(System.AnsiStrings.StrPas(SearchCat));
    ...
    begin
      ...
      begin
        AATableName := System.AnsiStrings.StrPas(SearchTableName);
    ...
    begin
      ...
      begin
        ...
        begin
          wsTableName := WideString(System.AnsiStrings.StrPas(SearchTableName));
        ...
        begin
          ...
          begin
            ...
            begin
              if not bUnicodeApi then
              begin
                if (System.AnsiStrings.StrLen(Schema) > 0) then
...
begin
  ...
  if Assigned(fSqlConnectionOdbc.fDbxTraceCallbackEven) then
    fSqlConnectionOdbc.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Misc, 'metadata (columns): (%s,%s,%s,%s,%d)', [
      System.AnsiStrings.StrPas(SearchCatalogName), System.AnsiStrings.StrPas(SearchSchemaName),
      System.AnsiStrings.StrPas(SearchTableName), System.AnsiStrings.StrPas(SearchColumnName),
...
//
    ...
    begin
      ...
      if Assigned(SearchColumnName) then
        i := System.AnsiStrings.StrLen(SearchColumnName)
      else
        i := 0;
      if (i > 0) and ((i <> Integer(System.AnsiStrings.StrLen(ColumnName))) or
        (System.AnsiStrings.StrLComp(SearchColumnName, ColumnName, i) <> 0)) then
      begin
        OdbcRetcode := SQLFetch(fHStmt);
        continue;
      end;
    ...
    begin
      ...
      begin
        ...
        if System.AnsiStrings.StrComp(aMetaColumn.fColumnName, ColumnName) = 0 then
...
var
  ...
  begin
    Result := string(System.AnsiStrings.StrPas(C) + AnsiChar(#1) + System.AnsiStrings.StrPas(S) + AnsiChar(#1) + System.AnsiStrings.StrPas(T) + AnsiChar(#1) + System.AnsiStrings.StrPas(I))
  end;
...
begin
  ...
  begin
    ...
    fSqlConnectionOdbc.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Misc, 'metadata (indexes): (%s,%s,%s,%s,%s)', [
      System.AnsiStrings.StrPas(SearchCatalogName), System.AnsiStrings.StrPas(SearchSchemaName),
      System.AnsiStrings.StrPas(SearchTableName), System.AnsiStrings.StrPas(SearchIndexName), sIndexType ]);
  end;
  {$ENDIF}
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if Trim(System.AnsiStrings.StrPas(OdbcPkName)) = '' then
            begin
              {$IFDEF _D9UP_}{$REGION 'COMMENTS'}{$ENDIF}
              {
              ...
              begin
                if MaxSet(fPkNameLenMax, 23, fStrLenLimit) then;//23=System.AnsiStrings.StrLen('[primary key - unnamed]'#0));
    ...
    // Get INDEX columns...
      ...
      begin
        ...
        begin
          ...
          begin // @dbg: System.AnsiStrings.StrPas(TableName)+' / '+System.AnsiStrings.StrPas(IndexName) +' / ' + System.AnsiStrings.StrPas(IndexColumnName)
            if ( (IndexName = nil) or (IndexName^ = cNullAnsiChar) ) // skip all unnamed indexes
              or
               ( (IndexName <> nil) and (aMetaTable.fPrimaryKeyColumn1 <> nil)
                 and (System.AnsiStrings.AnsiStrComp(IndexName, PAnsiChar(aMetaTable.fPrimaryKeyColumn1.fIndexName)) = 0)
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        sQuoteChar := System.AnsiStrings.StrPas(PAnsiChar(@sQuoteChar[cbIndexName+1]));
                        //cbIndexName := AnsiPos(AnsiString('+'), sQuoteChar);
                        cbIndexName := PosChar(AnsiChar('+'), sQuoteChar);
...
begin
  ...
  if Assigned(fSqlConnectionOdbc.fDbxTraceCallbackEven) then
    fSqlConnectionOdbc.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Misc, 'metadata (procedures): (%s,%d)', [
      System.AnsiStrings.StrPas(ProcedureName), Integer(ProcType) ]);
  {$ENDIF}
...
begin
  ...
  if Assigned(fSqlConnectionOdbc.fDbxTraceCallbackEven) then
    fSqlConnectionOdbc.DbxCallBackSendMsgFmt(cTDBXTraceFlags_Misc, 'metadata (params:enter): (%s,%s,%s,%s)', [
      System.AnsiStrings.StrPas(SearchCatalogName),  System.AnsiStrings.StrPas(SearchSchemaName),
      System.AnsiStrings.StrPas(SearchProcedureName), System.AnsiStrings.StrPas(SearchParamName) ]);
  {$ENDIF}
...
//
    ...
    begin
      ...
      begin
        vSearchCatalogName := AnsiUpperCase(System.AnsiStrings.StrPas(SearchCatalogName));
        SearchCatalogName := PAnsiChar(vSearchCatalogName);
      end;
      if (SearchSchemaName <> nil) and (SearchSchemaName^ <> cNullAnsiChar) then
      begin
        vSearchSchemaName := AnsiUpperCase(System.AnsiStrings.StrPas(SearchSchemaName));
        SearchSchemaName := PAnsiChar(vSearchSchemaName);
      end;
      if (SearchProcedureName <> nil) and (SearchProcedureName^ <> cNullAnsiChar) then
      begin
        vSearchProcedureName := AnsiUpperCase(System.AnsiStrings.StrPas(SearchProcedureName));
      ...
      begin
        vSearchParamName := AnsiUpperCase(System.AnsiStrings.StrPas(SearchParamName));
...
//    v_CHAR_OCTET_LENGTH := 0;
    ...
    begin
      ...
      if Assigned(SearchParamName) then
        i := System.AnsiStrings.StrLen(SearchParamName)
      else
        i := 0;
      if (i > 0) and ((i <> Integer(System.AnsiStrings.StrLen(ProcColumnName))) or
        (System.AnsiStrings.StrLComp(SearchParamName, ProcColumnName, i) <> 0)) then
      begin
        OdbcRetcode := SQLFetch(fHStmt);
        {$IFDEF _TRACE_CALLS_}
        //  inc(iFetchCount);
