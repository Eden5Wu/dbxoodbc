...
type
  TForm1 = class(TForm)
    ...
    procedure btn_open_query_textClick(Sender: TObject);
...
//  Exit;
  dbx_dbf_connect.DBFConnect(SQLConnection,
    ...
    // AdditionalOptions
      '',
//      'coEmptyStrParam=1', // update empty string '' into NULL
    // bUnicodeOdbcApi
...
//begin
//  mem_sql_text.Lines.Text := cbx_tables.Text;
//end;
var
  sObject: string;
begin
  ...
  if (FQuoteChar <> '') and (Pos(FQuoteChar, sObject) <= 0)
    and (Pos(' ', sObject) > 0) then
  begin
    sObject := FQuoteChar
...
procedure TForm1.btn_open_query_textClick(Sender: TObject);
var
  ws: WideString;
...
begin
  ...
  SQLQuery.Close;
  LoadStringFromFileW('..\DB\sql_text_unicode.txt', ws);
  SQLQuery.SQL.Text := '';
  SQLQuery.SQL.Text := ws; // work correctly for:
  mem_sql_text.Lines.Text := ws;
  //SQLQuery.Open;
...
procedure TForm1.pc_logChange(Sender: TObject);
begin
...
begin
  // @test: cds.fieldbyname('CRDACCNAME').Clear;
  mem_log.Lines.Clear;
end;
...
begin
  ...
  try
    ...
    begin
      // Delphi 2007 Down: List[i] - contained correctly table name, but mem_log is not unicode...
      sTable := List[i];
      cbx_tables.Items.Add(sTable);
    end;
  except
    Application.HandleException(Self);
  end;
  if cbx_tables.Items.Count = 0 then
    cbx_tables.Items.Text := 'table_demo';
