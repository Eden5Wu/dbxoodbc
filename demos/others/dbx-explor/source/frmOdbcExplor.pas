...
const
  c_version = {$i OdbcExplorVersion.inc};
...
{$IFDEF _D10UP_}
   ...
   {$IFDEF _D11UP_}
   //todo:...
   TISQLConnection25H = TObject;
   TISQLCursor = TObject;
   TISQLMetaData = TObject;
   {$ELSE}
...
procedure DecodeStringToBinary(const sInput: AnsiString; Output: TMemoryStream; out pFieldType: TFieldType);
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Base64DecodeStream(ms, msDeCompressed);
      end
      else
      begin
        msDeCompressed := ms;
...
procedure EncodeBinaryToString(Input: TStream; out sOutput: AnsiString; AFieldType: TFieldType;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if bEncodeBase64 then
          Base64EncodeStream(msCompressed, ms)
        else
        begin
          FreeAndNil(ms);
          ms := msCompressed;
...
begin
  ...
  SQLConnection1.DriverName := {DbxXXDrv.pas}DbxOODBCDriverName;
  ...
  //SQLConnection1.Params.Values[TRIMCHAR] := 'True';     // 'Trim Char'
  //SQLConnection1.Params.Values[ROWSETSIZE_KEY] := '20'; // 'RowsetSize'
  //SQLConnection1.Params.Values[PREPARESQL] := 'False'; // 'Prepare SQL'
  //SQLConnection1.Params.Values[SQLConnection.DriverName + ' TransIsolation'] := 'ReadCommited';
  //

  SQLConnection1.GetDriverFunc := GetDriverFunc({bUnicodeDriver}False);
...
// *** CONNECTION STRING ***
  ...
  {$IF CompilerVersion > 14.01}
     {$IF CompilerVersion >= 18.50}
       // Delphi 2007 Up
       SQLConnection1.Params.Values[DATABASENAME_KEY]  := '?';
       SQLConnection1.Params.Values[CONNECTION_STRING] := sConString;
       //if IsDbxDrvFixed then
       //  SQLConnection1.Params.Values[CUSTOM_INFO] := string(cConnectionOptionsNames[coConnectionString]) + '=' + sConString;
     {$ELSE}
  ...
  {$ELSE}
     ...
     begin
       SQLConnection1.Params.Values[DATABASENAME_KEY]  := '?';
       //SQLConnection1.Params.Values['Custom String'] := string(cConnectionOptionsNames[coConnectionString]) + '=' + sConString;
       SQLConnection1.Params.Values['ConnectionString'] := sConString;
     end
     else
     begin
       if Length(sConnectionString) > 255 then
         SetLength(sConnectionString, 255); // Buffer overflow AV protect :(
       SQLConnection1.Params.Values[DATABASENAME_KEY]  := sConnectionString;
     end;
  {$IFEND}
  //*)
