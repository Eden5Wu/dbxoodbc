...
{Appends the source text to the destination and returns the new destination
 position}
function AppendStringToBuffer(const ASource, ADestination: PAnsiChar; ACount: Cardinal): PAnsiChar; {+} overload; {+.}
begin
...
{+}
function AppendStringToBuffer(const ASource: AnsiString; const ADestination: PAnsiChar): PAnsiChar; {+} overload; {+.}
begin
  Result := AppendStringToBuffer(PAnsiChar(ASource), ADestination, Length(ASource));
end;
{+.}
...
begin
  ...
  begin
    ...
    {+}
    Result := AppendStringToBuffer(' Addr: ' + DWordToStr(DWORD(Pointer(AClass))), Result);
    {+.}
  end
  else
  begin
