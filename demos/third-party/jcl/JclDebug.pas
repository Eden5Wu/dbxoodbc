...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: #Date:: 2012-05-16 22:09:59 +0300 (Ср, 16 май 2012)                        $ }
{ Revision:      #Rev:: 3795                                                                     $ }
...
{+}
function InsertDebugDataIntoExecutableFileAsRes(const ExecutableFileName,
  MapFileName: TFileName): Boolean; overload;
function InsertDebugDataIntoExecutableFileAsRes(const ExecutableFileName: TFileName;
  BinDebug: TJclBinDebugGenerator): Boolean; overload;
{+.}
...
{+}
procedure JclRemoveExceptStackList(TID: DWORD);
{+.}
...
const
  UnitVersioning: TUnitVersionInfo = (
    ...
    Date: '#Date: 2012-05-16 22:09:59 +0300 (Ср, 16 май 2012) $';
...
function InsertDebugDataIntoExecutableFileAsRes(const ExecutableFileName,
  MapFileName: TFileName): Boolean;
var
  BinDebug: TJclBinDebugGenerator;
begin
  BinDebug := TJclBinDebugGenerator.Create(MapFileName);
  try
    Result := InsertDebugDataIntoExecutableFileAsRes(ExecutableFileName, BinDebug);
  finally
    BinDebug.Free;
  end;
end;
...
function InsertDebugDataIntoExecutableFileAsRes(const ExecutableFileName: TFileName;
  BinDebug: TJclBinDebugGenerator): Boolean;
var
  ...
  (*
  var
    JclPeImage: TJclPeImage;
    RL: TJclPeRootResourceList;
    RI: TJclPeResourceItem;
  //*)
  begin
    (* // todo: ...
    JclPeImage := TJclPeImage.Create();
    JclPeImage.AttachLoadedModule(DWord(ImageStream.Memory));
    if JclPeImage.IsBrokenFormat then
      Exit;
    RL := JclPeImage.ResourceList;
    if RL = nil then
      Exit;
    if RL.FindResource(rtRCData, JclDbgDataResName) <> nil then
      Exit;
    //RI := JclPeImage.

    //*)
    //
    // Save the EurekaLog data into the %JclDbgDataResName% Win32 PE resource...
    // "EurecaLog: EResource.pas"
    //
    {$IFDEF _EurecaLog_}
    WriteResourceInStream(PEStream, JclDbgDataResName,
      ResStream.Memory, ResStream.Size);
    {$ENDIF _EurecaLog_}
  end;

begin
  Result := BinDebug.Stream <> nil;
  if not Result then
    Exit;

  { BinDebug.DataStream -> Res }
  ...
  try
    try
      ...
      //Assert(Sections <> nil);
      // Check whether there is not a section with the name already. If so, return True (#0000069)
      if FindResource(DWord(ImageStream.Memory), JclDbgDataResName, RT_RCDATA ) <> 0 then
      begin
        Result := True;
        Exit;
      end;
      {$IFDEF _EurecaLog_}
      WriteResourceSection(ImageStream, BinDebug.DataStream);
...
begin
  SetLength(Buff, 2048);
  iLen := GetModuleFileName(HModule, PAnsiChar(Buff), Length(Buff));
  SetLength(Buff, iLen);
  Result := Buff;
end;
{+.}
...
begin
  ...
  begin
    {+} // debug:
    //OutputDebugString(PChar(ModuleFileName(Module)));
    {+.}
...
{+}
type
  TDataMemoryStream = class(TMemoryStream)
    procedure SetData(Ptr: Pointer; Size: Longint);
  end;

procedure TDataMemoryStream.SetData(Ptr: Pointer; Size: Longint);
begin
  SetPointer(Ptr, Size);
end;
{+.}
...
var
  ...
  {+}
  function LoadResourceToStream(Module: THandle; const ResName, ResType: PChar): Boolean;
  var
    InfoBlock: HRSRC;
  ...
  begin
    ...
    if (InfoBlock <> 0) then
    begin
      ResSize := SizeofResource(Module, InfoBlock);
      if ResSize > 0 then
      begin
        GlobalMemoryBlock := LoadResource(Module, InfoBlock);
        if (GlobalMemoryBlock <> 0) then
        begin
          pData := LockResource(GlobalMemoryBlock);
          FStream := TDataMemoryStream.Create;
          TDataMemoryStream(FStream).SetData(pData, ResSize);
          FStream.Position := 0;
          Result := True;
        end;
      end;
...
begin
  ...
  begin
    {+}
    //ShowMessage('JCL:Exception:01: not found JCLDEBUG info: ' + GetModulePath(Module));
    {$IFDEF WIN32}
    {
    //JclDbgDataResName
    ms := TDataMemoryStream.Create;
    try
      try
        ms.LoadFromFile(GetModulePath(Module));
        Result := (PeMapImgFindSection(PeMapImgNtHeaders(ms.Memory), JclDbgDataResName) <> nil);
        if Result then
        begin
          ShowMessage('JCL:Exception:02: WOW !!! JCLDEBUG info');
          FStream := TJclPeSectionStream.Create(Module, JclDbgDataResName);
        end
        else
          ShowMessage('JCL:Exception:02: not found JCLDEBUG info: ' + GetModulePath(Module));
      except
      end;
    finally
      ms.Free;
    ...
    {+.}
      {+}
      //
      // search and load debug information from resources
      //
      Result := LoadResourceToStream(Module, JclDbgDataResName, RT_RCDATA);
      if Result then
      begin
        VerifyFileName := True;
      end;
...
var
  ...
  const
    cNameLimit = 32;
    cNameLimit2 = cNameLimit div 2 -1;
  function TruncNameToLimit(const S: string): string;
  var
    i: Integer;
  begin
    Result := S;
    i := Length(S);
    if i <= cNameLimit then
      Result := S
    else
      Result := Copy(S, 1, cNameLimit2)+ '..' + Copy(S, Length(S) - cNameLimit2 + 1, cNameLimit2);
  end;
  {+.}
begin
  OffsetStr := '';
  ...
  begin
    ...
    {+}
      ...
      Insert(Format('{ %-32s }', [ExtractFileName(GetModulePath(Module))]), Result, 11 {$IFDEF CPU64}+8{$ENDIF});
      //Insert(Format('{ %-32s }', [TruncNameToLimit(ExtractFileName(GetModulePath(Module)))]), Result, 11 {$IFDEF CPU64}+8{$ENDIF});
    {+.}
  end;
end;
...
type
  ...
  private
    ...
    {+}
    procedure RemoveExceptStackInfo(TID: DWORD);
    {+.}
  public
    destructor Destroy; override;
...
{+}
procedure TJclGlobalStackList.RemoveExceptStackInfo(TID: DWORD);
var
  ReplacedObj: TObject;
begin
  with LockList do
  try
    ...
    ReplacedObj := FindObject(TID, TJclStackInfoList);
...
function JclValidateModuleAddress(Addr: Pointer): Boolean;
begin
  Result := {+}(Addr <> nil) and {+.} GlobalModulesList.ValidateAddress(Addr);
end;
...
procedure JclRemoveExceptStackList(TID: DWORD);
begin
  GlobalStackList.RemoveExceptStackInfo(TID);
end;
{+.}
