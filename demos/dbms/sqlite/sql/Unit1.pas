...
type
  TForm1 = class(TForm)
    ...
    procedure DSPUpdateError(Sender: TObject; DataSet: TCustomClientDataSet; E:
        EUpdateError; UpdateKind: TUpdateKind; var Response: TResolverResponse);
...
uses
  MidConst,
  dbx_constg, dbx_sqlite_connect, dbx_dbf_connect,
  DbxXXDrv; { critical }
...
begin
  ...
  sqlite3odbc_dll := ExtractFilePath(ParamStr(0)) + {}
    '..\odbc-driver\' + sqlite3odbc_dll;
...
{}
  ...
  begin
    ...
    if chk_conn_stg.Checked and FileExists(FConStorageFile) then
    begin
      if SQLConnectionLoadProperties(SQLConnection, FConStorageFile) then
      begin
        SQLConnection.VendorLib := 'odbc32.dll';
        if chk_direct_odbc.Checked then
    ...
    // MSACCESS
    //
    dbx_sqlite_connect.SQLiteConnect(SQLConnection,
      // db_file_name:
        ExtractFilePath(ParamStr(0)) + '..\db\dbdemos.sqlite',
      // DNS_NAME
        '',
      // DirectOdbc
...
begin
  // @test: cds.FieldByName('name').Clear;
  mem_log.Lines.Clear;
end;
...
procedure TForm1.DSPUpdateError(Sender: TObject; DataSet: TCustomClientDataSet;
...
begin
  ...
  try
    ...
    begin
      ...
      sTableName := List[i];
      if (sTableName <> '') and (sTableName[1] = FQuoteChar) then
      begin
        sTableName := Copy(sTableName, 2, Length(sTableName)-1);
        if (sTableName <> '') and (sTableName[Length(sTableName)] = FQuoteChar) then
          SetLength(sTableName, Length(sTableName)-1);
      end;
      if sTableName <> '' then
        cbx_tables.Items.Add(sTableName);
    end;
  except
