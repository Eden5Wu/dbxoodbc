...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: #Date:: 2012-03-04 20:12:39 +0200 (Вс, 04 мар 2012)                        $ }
{ Revision:      #Rev:: 3757                                                                     $ }
...
const
  UnitVersioning: TUnitVersionInfo = (
    ...
    Date: '#Date: 2012-03-04 20:12:39 +0200 (Вс, 04 мар 2012) $';
...
var
  ...
  ImportExceptionsHookedCpp: Boolean = False;
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFDEF BORLAND}
      if (not ImportExceptionsHookedCpp) and HasCppRtl then
      begin
        with TJclPeMapImgHooks do
          ImportExceptionsHookedCpp := ReplaceImport(GetCppRtlBase, kernel32, RaiseExceptionAddressCache, @HookedRaiseException);
      end;
      {$ENDIF BORLAND}
      ...
      begin
        Recursive := False;
        with TJclPeMapImgHooks do
          ImportExceptionsHooked := ReplaceImport(SystemBase, kernel32, RaiseExceptionAddressCache, @HookedRaiseException);
        Result := ImportExceptionsHooked;
      end;
    end;
    if Result then
    begin
...
begin
  {+}
  {$IFDEF BORLAND}
  if ImportExceptionsHookedCpp and HasCppRtl then
    ImportExceptionsHookedCpp := not TJclPeMapImgHooks.ReplaceImport(GetCppRtlBase, kernel32, @HookedRaiseException, @Kernel32_RaiseException);
  {$ENDIF BORLAND}
  if ImportExceptionsHooked then
  begin
    with TJclPeMapImgHooks do
      ImportExceptionsHooked := not ReplaceImport(SystemBase, kernel32, @HookedRaiseException, @Kernel32_RaiseException);
  end
  else
