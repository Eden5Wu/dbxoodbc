   ...
   "SqlExpr.pas"
       TSQLConnection
         - fixed clone connection.
           ...
           QC: 6315, 5867
         - added to Delphi6 connection handling options from Delphi7Up:
...
unit Dbx23Fix; // Delphi 2006 Down
...
{$IFNDEF CLR}
  {$IFDEF CONDITIONALEXPRESSIONS}
    ...
    {$IF Declared(CompilerVersion)}
      ...
      {$IFDEF _KYLIX_}
        {$IFDEF KYLIX_3UP}
          {.$DEFINE _UNSUPPORTED_} // TODO: UNCHECKED HOOKS
        {$ELSE}
          {$DEFINE _UNSUPPORTED_}  // TODO: UNCHECKED HOOKS
        {$ENDIF}
      {$ENDIF}
...
uses
  ...
  Types, Libc,
    {$ELSE}
    {$IFDEF POSIX}
  //Posix.SysMman, //Posix.Unistd, {Posix.String_,}
    {$ENDIF}
    {$ENDIF}
  {$ENDIF !MSWINDOWS}
  {$IFNDEF DELPHI_7UP}
...
type
  ...
  private
    ...
    FVendorLib: string;
    {$IFDEF DELPHI_7UP}{$IFNDEF _KYLIX_}
...
begin      // do not allow nested clones
  ...
  begin
    ...
    Status := TSQLConnectionH(SelfParent).FISQLConnection.getOption(
      TSQLConnectionOption(xeConnConnectionName), nil, 0, Len);
    if (Status<>0)or(Len<=0) then
      Len := 1024;
    SetLength(buf, Len);
    FillChar(buf[1], Len, #0);
    Status := TSQLConnectionH(SelfParent).FISQLConnection.getOption(
      TSQLConnectionOption(xeConnConnectionName), PChar(buf), Len, Len);
...
const
  ...
  //                   // ? DBCommon.pas: function IsMultiTableQuery()
  // D6.0.2:
...
procedure TSQLConnectionFix.DoConnect;
  ...
  begin
    ...
    if FParams.Values[OSAUTHENTICATION] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnOSAuthentication), LongInt(UpperCase(trim(FParams.Values[OSAUTHENTICATION])) = 'TRUE'));
    if FParams.Values[SERVERPORT] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnServerPort), LongInt(trim(FParams.Values[SERVERPORT])));
    if FParams.Values[MULTITRANSENABLED] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnMultipleTransaction), LongInt(UpperCase(trim(FParams.Values[MULTITRANSENABLED])) = 'TRUE'));
    if FParams.Values[TRIMCHAR] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnTrimChar), LongInt(UpperCase(trim(FParams.Values[TRIMCHAR])) = 'TRUE'));

    if FParams.Values[CUSTOM_INFO] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnCustomInfo), LongInt(trim(FParams.Values[CUSTOM_INFO])));
    if FParams.Values[CONN_TIMEOUT] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnTimeOut), LongInt(StrToInt(trim(FParams.Values[CONN_TIMEOUT]))));
    if FParams.Values[CONNECTION_STRING] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnConnectionString), LongInt(trim(FParams.Values[CONNECTION_STRING])));
    if FParams.Values[PREPARESQL] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnPrepareSQL), LongInt(UpperCase(trim(FParams.Values[PREPARESQL])) = 'TRUE'));
    if FParams.Values[DECIMALSEPARATOR] <> '' then
      FISQLConnection.SetOption(TSQLConnectionOption(xeConnDecimalSeparator), LongInt(trim(FParams.Values[DECIMALSEPARATOR])));
...
begin
  ...
  try
    ...
    FISQLConnection.getOption(TSQLConnectionOption(xeConnMaxActiveComm), @FMaxStmtsPerConn, Sizeof(Integer), PropSize);
...
{$ENDIF IFDEF DELPHI_7UP}
  ...
  begin
    ...
    P := PChar(Name);
    Status := TSQLConnectionH({DS.}{Get}InternalConnection).FISQLConnection.setOption(TSQLConnectionOption(xeConnQualifiedName), LongInt(P));
    if Status <> 0 then
      {DS.}SQLError(Status, exceptConnection);
    Status := TSQLConnectionH({DS.}{Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnQuotedObjectName), @buf, SizeOf(buf), Len);
    if Status <> 0 then
      {DS.}SQLError(Status, exceptConnection);
    Result := buf;
...
procedure TCustomSQLDataSetFixD7.OpenSchema;
  ...
  begin
    ...
    if ObjectName <> '' then
    begin
      Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.setOption(TSQLConnectionOption(xeConnQualifiedName), LongInt(ObjectName));
      if Status <> 0 then
        SQLError(Status, exceptConnection);
      {+}
      try
        Len := 0;
        Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnCatalogName), PChar(buf0), 0, Len);
      ...
      {+.}
      Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnCatalogName), PChar(buf0), Length(buf0), Len);
      if Status <> 0 then
        SQLError(Status, exceptConnection);
      {+}
      try
        Len := 0;
        Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnSchemaName), PChar(buf1), 0, Len);
      ...
      {+.}
      Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnSchemaName), PChar(buf1), Length(buf1), Len);
      if Status <> 0 then
        SQLError(Status, exceptConnection);
      ...
      try
        Len := 0;
        Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnObjectName), PChar(buf2), 0, Len);
      ...
      {+.}
      Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnObjectName), PChar(buf2), Length(buf2), Len);
      if Status <> 0 then
        SQLError(Status, exceptConnection);
    ...
    try
      ...
      begin
        ...
        Status := TSQLConnectionH({Get}InternalConnection).FISQLConnection.getOption(TSQLConnectionOption(xeConnObjectName), PChar(buf0), Length(buf0), Len);
        CatalogName := buf0;
      end;
    except
      //no open odbc driver and it driver not check buffer length or not return required buffer length
...
begin
  ...
  begin
    ...
    begin
      ...
      if (iUnits1 > (MaxBcdPrecision-4)) or (iUnits2 > MaxBcdScale) {$IF CompilerVersion >= 15.00}or FNumericMapping{$IFEND} then
      begin
        FType := ftFMTBcd;
        iFldType := fldFMTBCD;
...
begin
  ...
  {$ELSE}
  {$IFDEF POSIX}
  if mprotect(MSrc, SizeOf(TJumpInstruction), PROT_WRITE or PROT_EXEC) = 0 then
  {$ENDIF}
  {$ENDIF}
  try
     if Assigned(SaveJump) then
