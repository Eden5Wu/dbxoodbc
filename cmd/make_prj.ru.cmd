...
@rem # Описание
@rem # ======================
@rem # Компиляция :Delphi/Delphi.Net/C++Builder/Kylix" pas модулей/проектов.
@rem # Для работы нужен get_regval.exe.
...
@echo   "make_prj.cmd" - утилита сборки/компиляции проекта/модуля Delphi/Delphi.Net/C++Builder/Kylix>con
@echo ---------------------------------------------------------------------------------------------->con
...
@echo\>con
@echo        /C       - Clear - чистка текущей директории от: (dcu, obj, hpp, drc, ...)>con
@echo        /CA      - /C + remove  "cfg, dof, dsk" Files>con
@echo\>con
@echo   [2] "Project_Name|?"  -  Имя проекта/модуля или ? для установки переменных окружения.>con
@echo\>con
@echo   [3] "Options":>con
@echo          /c  - искать общий "%%SystemRoot%%\dx_common.cmd" описывающий общие настройки.>con
@echo\>con
@echo Примеры:>con
...
@echo   make_prj.cmd 6 project1.dpr /c - все настройки указанны в dx_common.cmd>con
@echo PS:>con
@echo   Для работы нужен "get_regval.exe">con
@echo ------------------------------------------------------------------------------------------>con
@goto L_EXIT
