...
// 2)...fixed a bug that did not works with the CBuilder libraries |
// 3)...removed many not needed functionalities                    |
// -----------------------------------------------------------------
...
type
  ...
  public
    constructor Create(AParent: TPEModule; const AHeader: TImageSectionHeader;
      rawData: pointer);
    destructor Destroy; override;
  ...
  {$IFDEF WIN32}
    {$IFNDEF Delphi16}
    PImageOptionalHeader32 = PImageOptionalHeader;
    {$ENDIF}
...
type
  TResourceNode = class
    count: Integer;
    nodes: array[0..1023] of record // Max 1024 nodes.
      id: string;
      intID: boolean;
  ...
  begin
    ...
    for n := 0 to SecNum - 1 do
      if SecH[n].VirtualAddress =
        H^.OptionalHeader.DataDirectory[2].VirtualAddress then Result := n;
  end;
...
begin
  ...
  DeltaSec := GetAlign(DWord(ResStream.Size) -
    DWord(Headers^.OptionalHeader.DataDirectory[2].Size), SecAlign);
  ...
  if (Headers^.OptionalHeader.DataDirectory[2].Size > 0) then
    Inc(Headers^.OptionalHeader.DataDirectory[2].Size, DeltaSec);
  ...
  for n := 0 to SecCount - 1 do
    ...
    begin
      if (SecHead[n].VirtualAddress > 0) then
        Inc(SecHead[n].VirtualAddress, DeltaSec);
      if (SecHead[n].PointerToRawData > 0) then
...
begin
  ...
  Inc(wstr);
  WideCharToMultiByte(CP_ACP, 0, WStr, Len, PAnsiChar(PChar(Result)), Len + 1, nil, nil);
  Inc(wstr, len);
  result := string(PAnsiChar(PChar(result)));
end;
...
begin
  ...
  for i := 1 to Length(s) do
    if not (AnsiChar(s[i]) in ['0'..'9']) then
    begin
      isNumeric := False;
      break
    end;
...
begin
  for i := 0 to fSectionList.Count - 1 do TObject(fSectionList[i]).Free;
  fSectionList.Clear;
...
begin
  ...
  begin
    ...
    fUninitializedDataSize := fSectionHeader.Misc.VirtualSize -
      fSectionHeader.SizeOfRawData;
  end;
...
var
  i: Integer;
begin
  for i := 0 to fDetailList.Count - 1 do TObject(fDetailList[i]).Free;
  fDetailList.Free;
  inherited;
end;
    ...
    begin
      ...
      begin
        ...
        details := TResourceDetails.Create(self, lang, name, tp,
          dataEntry^.Size, PAnsiChar(section.fRawData.Memory) +
...
begin
  ...
  for i := 1 to Length(nodes[idx].id) do
    if not (AnsiChar(nodes[idx].id[i]) in ['0'..'9']) then
    begin
      result := False;
      break
    end;
