...
uses
  ...
  Posix.Unistd, Posix.Stdio, Posix.Pthread,
  {$ENDIF}{$ENDIF}
  Classes,
  SysUtils,
  {$IFDEF _D11UP_}
...
type
  pTTransactionDescBase = pTTransactionDesc;
  TSTMTParamTypeBase = TSTMTParamType;
  TSQLCursorOptionBase = TSQLCursorOption;

{$IFDEF UNICODE}
procedure OutputDebugString(const S: string); {$IFDEF _INLINE_} inline; {$ENDIF} overload;
{$ENDIF}
procedure OutputDebugString(const S: AnsiString); {$IFDEF _INLINE_} inline; {$ENDIF} overload;
procedure OutputDebugString(const S: WideString); {$IFDEF _INLINE_} inline; {$ENDIF} overload;
...
begin
  ...
  try
    ...
    on e: Exception do
    begin
      OutputDebugString('*** TRACCER INTERNAL ERROR: (function ValueToStr). Detail: ' + e.Message);
    end;
  end;
end;
  ...
  try
    ...
    case pVarRec.VType of
      ...
      vtChar   :
        sV := pVarRec.VChar;
      vtExtended:
        if Assigned(pVarRec.VExtended) then
          sV := AnsiString(FloatToStr(pVarRec.VExtended^))
        else
          sV := '';
      vtPointer:
      ...
      {$IFDEF _D12UP_}
      vtUnicodeString:
        if Assigned(pVarRec.vUnicodeString) then
    ...
    on e: Exception do
    begin
      OutputDebugString('*** TRACCER INTERNAL ERROR: (function ArrayToStr). Detail: ' + e.Message);
    end;
  end;
end;
