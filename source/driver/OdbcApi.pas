{
  Delphi translation of the Microsoft ODBC API headers, ODBC version 3.8, 3.51, 2x
...
{$IFDEF LINUX}
  {$UNDEF ODBC_STDCALL} // cdecl
{$ENDIF}

{$IFDEF MSWINDOWS}
  {$DEFINE MSWINDOWS_OR_LINUX}
{$ELSE}
  {$IFDEF LINUX}
    {$DEFINE MSWINDOWS_OR_LINUX}
  {$ELSE}
    {$UNDEF MSWINDOWS_OR_LINUX}
  {$ENDIF}
{$ENDIF}
...
{$ENDIF}
  ...
  var _2: WideChar;
  _3: Longint
  ): Retcode; {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  _1: Retcode;
  _2: Retcode); {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  Longint; {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  _1: Longint): Retcode; {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  dwValue: LongWord { In seconds }
  ):{$IFDEF MSWINDOWS}BOOL{$ELSE}LongBool{$ENDIF};
  {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$ENDIF}
  Longint; {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF}; { In Milliseconds(!) }
...
{$ENDIF}
  var Args: ODBC_VS_ARGS); {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF};
...
{$IFDEF DynamicOdbcImport}
  ...
  public
    ...
    property VendorLib: AnsiString read fVendorLib;
  ...
  // ODBC32DLL
    ...
    //ODBCINST
    function ODBCSetTryWaitValue(dwValue: LongWord): Boolean;
...
{$ELSE}
  ...
  public
    ...
    property OdbcDriverLevel: Integer read fOdbcDriverLevel write SetOdbcDriverLevel;
    property VendorLib: AnsiString read GetModuleName;
...
{$IFDEF _iOdbcApi_}
  ...
  private
    ...
    {$IFDEF DynamicOdbcImport}
    procedure CheckAvailableProxy; {$IF CompilerVersion >= 20.00} inline; {$IFEND}
    {$ENDIF}
  { IOdbcApi }
  protected
   // MACROs
...
{$ELSE}
  sysodbclib: AnsiString = 'libodbc.so';
{$ENDIF}
{$ENDIF}
...
function SQLColAttributeString;
   {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF}; external sysodbclib name 'SQLColAttribute';
...
function SQLGetInfoString;
   {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF}; external sysodbclib name 'SQLGetInfo';
...
const
  ...
  ODBCINST = {$IFDEF MSWINDOWS} 'odbcint.dll'{$ELSE} 'libodbcinst.so'{$ENDIF};
...
function TraceOpenLogFile;
   {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF}; external ODBCTRAC;
...
function ODBCGetTryWaitValue;
   {$IFDEF ODBC_STDCALL}stdcall{$ELSE}cdecl{$ENDIF}; external ODBCINST;
...
function LoadOdbcDriverManager(LibraryName: PAnsiChar; UnicodePriority: Boolean): TOdbcApiProxy;
var
  sVendorLib, sLibNames, sLib, sLibKey: AnsiString;
...
const
  cLibsSeparator: AnsiChar = ';';
  cSqlAttrOdbcVerSeparator: AnsiChar = ':';

  function ExtractNextLibName: AnsiString;
  var
    i: integer;
  begin
    i := iPos;
  ...
  function DoGetProcAddress(Module: HMODULE; const ProcName: AnsiString): Pointer; {$IF CompilerVersion >= 20.00} inline; {$IFEND}
  begin
    {$IFDEF MSWINDOWS_OR_LINUX}
    Result := GetProcAddress(Module, PAnsiChar(ProcName));
    {$ELSE}
    Result := GetProcAddress(Module, PChar(string(ProcName))); // POSIX (MACOS = WideString)
    {$ENDIF}
  end;

  function DoGetProcAddressEx(Module: HMODULE; const sProcName: AnsiString; PtrA: PPointer;
    PtrW: PPointer = nil): Boolean;
  begin
    PtrA^ := DoGetProcAddress(Module, sProcName);
    if Assigned(PtrW) then
    begin
      PtrW^ := DoGetProcAddress(Module, sProcName+'W');
...
begin
  Result := nil;
  sLibNames := Trim(System.AnsiStrings.StrPas(LibraryName));
  sVendorLib := sLibNames;
  if sLibNames = '' then
    //raise EOdbcApi.Create('Not defined Api Library.');
    sLibNames := sysodbclib;
  vHModule := 0;
  //iLastOSError := 0;
  vOdbcDriverLevel := -1;
  ...
  try
    ...
    repeat
      ...
      iTmp := PosChar(cSqlAttrOdbcVerSeparator, sLib);
         ...
         begin
           ...
           begin
             ...
             if (Length(sEnv)>9) and
               (System.AnsiStrings.StrLComp(PAnsiChar(sEnv), 'pbidp', 5) = 0) and ( ExtractFileExt(sEnv) = '.dll')
    ...
    try
      ...
      // sysodbclib
        // sql.h
        DoGetProcAddressEx(hModuleApi, 'SQLAllocHandle', @@SQLAllocHandleA);
        if not Assigned(SQLAllocHandleA) then
        begin
          DoGetProcAddressEx(hModuleApi, 'SQLAllocConnect', @@SQLAllocConnectA);  // ODBC 2
          DoGetProcAddressEx(hModuleApi, 'SQLAllocEnv', @@SQLAllocEnvA);        // ODBC 2
          DoGetProcAddressEx(hModuleApi, 'SQLAllocStmt', @@SQLAllocStmtA);      // ODBC 2
        end;
        DoGetProcAddressEx(hModuleApi, 'SQLBindCol', @@SQLBindColA);
        DoGetProcAddressEx(hModuleApi, 'SQLBindParam', @@SQLBindParamA);
        DoGetProcAddressEx(hModuleApi, 'SQLCancel', @@SQLCancelA);
        DoGetProcAddressEx(hModuleApi, 'SQLCloseCursor', @@SQLCloseCursorA);
        DoGetProcAddressEx(hModuleApi, 'SQLColAttribute', @@SQLColAttributeA, @@SQLColAttributeW);
        DoGetProcAddressEx(hModuleApi, 'SQLColumns', @@SQLColumnsA, @@SQLColumnsW);
        DoGetProcAddressEx(hModuleApi, 'SQLConnect', @@SQLConnectA, @@SQLConnectW);
        DoGetProcAddressEx(hModuleApi, 'SQLCopyDesc', @@SQLCopyDescA);
        DoGetProcAddressEx(hModuleApi, 'SQLDataSources', @@SQLDataSourcesA, @@SQLDataSourcesW);
        DoGetProcAddressEx(hModuleApi, 'SQLDescribeCol', @@SQLDescribeColA, @@SQLDescribeColW);
        DoGetProcAddressEx(hModuleApi, 'SQLDisconnect', @@SQLDisconnectA);
        DoGetProcAddressEx(hModuleApi, 'SQLEndTran', @@SQLEndTranA);
        DoGetProcAddressEx(hModuleApi, 'SQLError', @@SQLErrorA, @@SQLErrorW);
        DoGetProcAddressEx(hModuleApi, 'SQLExecDirect', @@SQLExecDirectA, @@SQLExecDirectW);
        DoGetProcAddressEx(hModuleApi, 'SQLExecute', @@SQLExecuteA);
        DoGetProcAddressEx(hModuleApi, 'SQLFetch', @@SQLFetchA);
        DoGetProcAddressEx(hModuleApi, 'SQLFetchScroll', @@SQLFetchScrollA);
        DoGetProcAddressEx(hModuleApi, 'SQLFreeConnect', @@SQLFreeConnectA);
        DoGetProcAddressEx(hModuleApi, 'SQLFreeEnv', @@SQLFreeEnvA);
        DoGetProcAddressEx(hModuleApi, 'SQLFreeHandle', @@SQLFreeHandleA);
        DoGetProcAddressEx(hModuleApi, 'SQLFreeStmt', @@SQLFreeStmtA);
        DoGetProcAddressEx(hModuleApi, 'SQLGetConnectAttr', @@SQLGetConnectAttrA, @@SQLGetConnectAttrW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetConnectOption', @@SQLGetConnectOptionA, @@SQLGetConnectOptionW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetCursorName', @@SQLGetCursorNameA, @@SQLGetCursorNameW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetData', @@SQLGetDataA);
        DoGetProcAddressEx(hModuleApi, 'SQLGetDescField', @@SQLGetDescFieldA, @@SQLGetDescFieldW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetDescRec', @@SQLGetDescRecA, @@SQLGetDescRecW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetDiagField', @@SQLGetDiagFieldA, @@SQLGetDiagFieldW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetDiagRec', @@SQLGetDiagRecA, @@SQLGetDiagRecW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetEnvAttr', @@SQLGetEnvAttrA);
        DoGetProcAddressEx(hModuleApi, 'SQLGetFunctions', @@SQLGetFunctionsA);
        DoGetProcAddressEx(hModuleApi, 'SQLGetInfo', @@SQLGetInfoA, @@SQLGetInfoW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetStmtAttr', @@SQLGetStmtAttrA, @@SQLGetStmtAttrW);
        DoGetProcAddressEx(hModuleApi, 'SQLGetStmtOption', @@SQLGetStmtOptionA);
        DoGetProcAddressEx(hModuleApi, 'SQLGetTypeInfo', @@SQLGetTypeInfoA, @@SQLGetTypeInfoW);
        DoGetProcAddressEx(hModuleApi, 'SQLNumResultCols', @@SQLNumResultColsA);
        DoGetProcAddressEx(hModuleApi, 'SQLParamData', @@SQLParamDataA);
        DoGetProcAddressEx(hModuleApi, 'SQLPrepare', @@SQLPrepareA, @@SQLPrepareW);
        DoGetProcAddressEx(hModuleApi, 'SQLPutData', @@SQLPutDataA);
        DoGetProcAddressEx(hModuleApi, 'SQLRowCount', @@SQLRowCountA, @@SQLRowCountW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetConnectAttr', @@SQLSetConnectAttrA, @@SQLSetConnectAttrW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetConnectOption', @@SQLSetConnectOptionA, @@SQLSetConnectOptionW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetCursorName', @@SQLSetCursorNameA, @@SQLSetCursorNameW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetDescField', @@SQLSetDescFieldA, @@SQLSetDescFieldW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetDescRec', @@SQLSetDescRecA);
        DoGetProcAddressEx(hModuleApi, 'SQLSetEnvAttr', @@SQLSetEnvAttrA);
        DoGetProcAddressEx(hModuleApi, 'SQLSetParam', @@SQLSetParamA);
        DoGetProcAddressEx(hModuleApi, 'SQLSetStmtAttr', @@SQLSetStmtAttrA, @@SQLSetStmtAttrW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetStmtOption', @@SQLSetStmtOptionA);
        DoGetProcAddressEx(hModuleApi, 'SQLSpecialColumns', @@SQLSpecialColumnsA, @@SQLSpecialColumnsW);
        DoGetProcAddressEx(hModuleApi, 'SQLStatistics', @@SQLStatisticsA, @@SQLStatisticsW);
        DoGetProcAddressEx(hModuleApi, 'SQLTables', @@SQLTablesA, @@SQLTablesW);
        DoGetProcAddressEx(hModuleApi, 'SQLTransact', @@SQLTransactA);
        // sqlext.h
        DoGetProcAddressEx(hModuleApi, 'SQLDriverConnect', @@SQLDriverConnectA, @@SQLDriverConnectW);
        DoGetProcAddressEx(hModuleApi, 'SQLBrowseConnect', @@SQLBrowseConnectA, @@SQLBrowseConnectW);
        DoGetProcAddressEx(hModuleApi, 'SQLBulkOperations', @@SQLBulkOperationsA);
        DoGetProcAddressEx(hModuleApi, 'SQLColAttributes', @@SQLColAttributesA, @@SQLColAttributesW);
        DoGetProcAddressEx(hModuleApi, 'SQLColumnPrivileges', @@SQLColumnPrivilegesA, @@SQLColumnPrivilegesW);
        DoGetProcAddressEx(hModuleApi, 'SQLDescribeParam', @@SQLDescribeParamA);
        DoGetProcAddressEx(hModuleApi, 'SQLExtendedFetch', @@SQLExtendedFetchA);
        DoGetProcAddressEx(hModuleApi, 'SQLForeignKeys', @@SQLForeignKeysA, @@SQLForeignKeysW);
        DoGetProcAddressEx(hModuleApi, 'SQLMoreResults', @@SQLMoreResultsA);
        DoGetProcAddressEx(hModuleApi, 'SQLNativeSql', @@SQLNativeSqlA, @@SQLNativeSqlW);
        DoGetProcAddressEx(hModuleApi, 'SQLNumParams', @@SQLNumParamsA);
        DoGetProcAddressEx(hModuleApi, 'SQLParamOptions', @@SQLParamOptionsA);
        DoGetProcAddressEx(hModuleApi, 'SQLPrimaryKeys', @@SQLPrimaryKeysA, @@SQLPrimaryKeysW);
        DoGetProcAddressEx(hModuleApi, 'SQLProcedureColumns', @@SQLProcedureColumnsA, @@SQLProcedureColumnsW);
        DoGetProcAddressEx(hModuleApi, 'SQLProcedures', @@SQLProceduresA, @@SQLProceduresW);
        DoGetProcAddressEx(hModuleApi, 'SQLSetPos', @@SQLSetPosA);
        DoGetProcAddressEx(hModuleApi, 'SQLTablePrivileges', @@SQLTablePrivilegesA, @@SQLTablePrivilegesW);
        DoGetProcAddressEx(hModuleApi, 'SQLDrivers', @@SQLDriversA, @@SQLDriversW);
        DoGetProcAddressEx(hModuleApi, 'SQLBindParameter', @@SQLBindParameterA);
        DoGetProcAddressEx(hModuleApi, 'SQLAllocHandleStd', @@SQLAllocHandleStdA);
        DoGetProcAddressEx(hModuleApi, 'SQLSetScrollOptions', @@SQLSetScrollOptionsA, @@SQLSetScrollOptionsW);

        fSystemODBCManager := CompareText(fModuleName, sysodbclib) = 0;
        ...
        begin
          ...
          begin
            DoGetProcAddressEx(hModuleApiTrac, 'TraceOpenLogFile', @@TraceOpenLogFileA);
            DoGetProcAddressEx(hModuleApiTrac, 'TraceCloseLogFile', @@TraceCloseLogFileA);
            DoGetProcAddressEx(hModuleApiTrac, 'TraceReturn', @@TraceReturnA);
            DoGetProcAddressEx(hModuleApiTrac, 'TraceVersion', @@TraceVersionA);
            DoGetProcAddressEx(hModuleApiTrac, 'TraceVSControl', @@TraceVSControlA);
            DoGetProcAddressEx(hModuleApiTrac, 'FireVSDebugEvent', @@FireVSDebugEventA);
          end;
          //ODBCINST
          ...
          begin
            DoGetProcAddressEx(hModuleApiInst, 'ODBCSetTryWaitValue', @@ODBCSetTryWaitValueA);
            DoGetProcAddressEx(hModuleApiInst, 'ODBCGetTryWaitValue', @@ODBCGetTryWaitValueA);
          end;
        end;
...
begin
  ...
  try
    ...
    if (not fLockLib) and (fApiRefCount <= 0) then
      fOdbcApiCache.Remove(fModuleName, True); // call OdbcApi.Free (by method Dispose)
    {$ENDIF}
  finally
    fOdbcApiCache.EndLock;
  end;
...
begin
  ...
  begin
    ...
    wszConnStrIn := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szConnStrIn)));
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(@szConnStrOut), PAnsiChar(sConnStrOut));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(CharacterAttribute), PAnsiChar(sCharacterAttribute));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
     ...
     FillChar(PAnsiChar(PWideChar(wszDesc))^, cbDescMax*4, 0);
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(rgbDesc), PAnsiChar(sDesc));
    end;
  end
  else
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszCatalogName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szCatalogName)));
      //cbCatalogName := SQL_NTS; // or Length(wszCatalogName) + 1;{???: +1}
    wszSchemaName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szSchemaName)));
      //cbSchemaName := Length(wszSchemaName) + 1;
    wszTableName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szTableName)));
      //cbTableName := Length(wszTableName) + 1;
    wszColumnName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szColumnName)));
...
begin
  ...
  begin
    ...
    begin
      wszCatalogName := WideString(System.AnsiStrings.StrPas(CatalogName));
    ...
    begin
      wszSchemaName := WideString(System.AnsiStrings.StrPas(SchemaName));
    ...
    begin
      wszTableName := WideString(System.AnsiStrings.StrPas(TableName));
    ...
    begin
      wszColumnName := WideString(System.AnsiStrings.StrPas(ColumnName));
...
begin
  ...
  begin
    ...
    begin
      wszServerName := WideString(System.AnsiStrings.StrPas(ServerName));
    ...
    begin
      wszUserName := WideString(System.AnsiStrings.StrPas(UserName));
    ...
    begin
      wszAuthentication := WideString(System.AnsiStrings.StrPas(Authentication));
...
begin
  ...
  begin
    ...
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(@ServerName), PAnsiChar(sServerName));
    end;
    if BufferLength1 > 0 {= SQL_NTS} then
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(@Description), PAnsiChar(sDescription));
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(ColumnName, PAnsiChar(sColumnName));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      wszConnStrIn := WideString(System.AnsiStrings.StrPas(szConnStrIn));
    ...
    begin
      ...
      if Length(sConnStrOut) >= cbConnStrOutMax then
        SetLength(sConnStrOut, cbConnStrOutMax-1);
      System.AnsiStrings.StrCopy(szConnStrOut, PAnsiChar(sConnStrOut));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    if cbDriverDescMax > 0 then
      FillChar(PAnsiChar(PWideChar(wszDriverDesc))^, cbDriverDescMax*2, 0);
    pcbDriverDesc := 0;
    ...
    if cbDrvrAttrMax > 0 then
      FillChar(PAnsiChar(PWideChar(wszDriverAttributes))^, cbDrvrAttrMax*2, 0);
    pcbDrvrAttr := 0;
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(szDriverDesc, PAnsiChar(sDriverDesc));
    end;
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(szDriverAttributes, PAnsiChar(sDriverAttributes));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    FillChar(PAnsiChar(PWideChar(wszSqlState))^, Length(wszSqlState)*2-2, AnsiChar('0'));
    if BufferLength < 0 then
      BufferLength := 0;
    SetLength(wszMessageText, BufferLength*4);
    if BufferLength > 0 then
      FillChar(PAnsiChar(PWideChar(wszMessageText))^, Length(wszSqlState), 0);
    TextLength := 0;
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(@SqlState), PAnsiChar(sSqlState));
      System.AnsiStrings.StrCopy(PAnsiChar(@MessageText), PAnsiChar(sMessageText));
    end;
  end
  else
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszStatementText := WideString(System.AnsiStrings.StrPas(StatementText));
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszPkCatalogName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szPkCatalogName)));
    wszPkSchemaName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szPkSchemaName)));
    wszPkTableName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szPkTableName)));
    wszFkCatalogName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szFkCatalogName)));
    wszFkSchemaName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szFkSchemaName)));
    wszFkTableName := WideString(System.AnsiStrings.StrPas(PAnsiChar(@szFkTableName)));
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(PAnsiChar(PWideChar(wszValuePtr))^, BufferLength*2, 0);
    ...
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(ValuePtr), PAnsiChar(sValuePtr));
    end;
  end
  else
  ...
  begin
    ...
    if bHandleWS then
      System.AnsiStrings.StrLCopy( PansiChar(ValuePtr), vValuePtr, BufferLength);
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(PAnsiChar(PWideChar(wszCursorName))^, BufferLength*2, 0);
      NameLength := 0;
    end
    else
      wszCursorName := '';
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(CursorName, PAnsiChar(sCursorName));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(PAnsiChar(PWideChar(wszValue))^, BufferLength*2, 0);
      StringLength := 0;
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(Value), PAnsiChar(sValue));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(PAnsiChar(PWideChar(wszName))^, BufferLength*2, 0);
    end
    else
    begin
      wszName := '';
    ...
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(@Name), PAnsiChar(sName));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(PAnsiChar(PWideChar(wszDiagInfo))^, BufferLength*2, 0);
    end
    else
    begin
      wszDiagInfo := '';
    ...
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(DiagInfo), PAnsiChar(sDiagInfo));
    end;
  end
  else
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    SetLength(wszSqlState, SizeOf(TSqlState)-1);
    FillChar(PAnsiChar(PWideChar(wszSqlState))^, Length(wszSqlState)*SizeOf(WideChar), 0);
    if BufferLength < 0 then
      BufferLength := 0;
    SetLength(wszMessageText, BufferLength*4);
    if BufferLength > 0 then
      FillChar(PAnsiChar(PWideChar(wszMessageText))^, Length(wszMessageText)*SizeOf(WideChar), 0);
    TextLength := 0;
    ...
    begin
      sSqlState := AnsiString(wszSqlState);
      sMessageText := AnsiString(wszMessageText);
      System.AnsiStrings.StrCopy(SqlState, PAnsiChar(sSqlState));
      System.AnsiStrings.StrCopy(MessageText, PAnsiChar(sMessageText));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if BufferLength > 0 then
        FillChar(PAnsiChar(PWideChar(wszInfoValuePtr))^, BufferLength, 0);
    ...
    begin
      ...
      System.AnsiStrings.StrCopy(PAnsiChar(InfoValuePtr), PAnsiChar(sInfoValuePtr));
    end;
  end
  else
...
begin
  ...
  begin
    //ANSI to Unicode mappings:
    wszSqlStrIn := Widestring(System.AnsiStrings.StrPas(PAnsiChar(@szSqlStrIn)));
    if cbSqlStrMax > 0 then
    begin
      SetLength(wszSqlStr, cbSqlStrMax);
      FillChar(PAnsiChar(PWideChar(wszSqlStr))^, cbSqlStrMax*2, 0);
    end
    else
      wszSqlStr := '';
    pcbSqlStr := 0;
    ...
    begin
      ...
      System.AnsiStrings.StrCopy( PAnsiChar(@szSqlStr), PAnsiChar(sSqlStr));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      wszCatalogName := WideString(System.AnsiStrings.StrPas(szCatalogName));
      szCatalogName := PAnsiChar(PWideChar(wszCatalogName));
    end;
    if Assigned(szSchemaName) then
    begin
      wszSchemaName := WideString(System.AnsiStrings.StrPas(szSchemaName));
      szSchemaName := PAnsiChar(PWideChar(wszSchemaName));
    end;
    if Assigned(szTableName) then
    begin
      wszTableName := WideString(System.AnsiStrings.StrPas(szTableName));
...
begin
  ...
  begin
    ...
    begin
      wszProcName := WideString(System.AnsiStrings.StrPas(szProcName));
      szProcName := PAnsiChar(PWideChar(wszProcName));
    end;

    if Assigned(szColumnName) then
    begin
      wszColumnName := WideString(System.AnsiStrings.StrPas(szColumnName));
...
begin
  ...
  begin
    ...
    if bHandleWS then
    begin
      wszValuePtr := WideString(System.AnsiStrings.StrPas(PAnsiChar(ValuePtr)));
...
begin
  ...
  begin
    ...
    begin
      wszCursorName := WideString(System.AnsiStrings.StrPas(CursorName));
...
begin
  ...
  begin
    ...
    begin
      wszValue := WideString(System.AnsiStrings.StrPas(PAnsiChar(Value)));
...
begin
  ...
  begin
    ...
    begin
      wszTableType := WideString(System.AnsiStrings.StrPas(TableType));
...
function TOdbcApiProxy.ODBCSetTryWaitValue(dwValue: LongWord): Boolean;
begin
  Result := False;
...
function TOdbcApiIntfImpl.ODBCSetTryWaitValue(dwValue: LongWord): Boolean;
begin
  {$IFDEF DynamicOdbcImport}
    CheckAvailableProxy;
