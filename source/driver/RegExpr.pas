...
{
     TRegExpr class library. Version: 0.952
...
{$IFDEF CONDITIONALEXPRESSIONS}
  {$DEFINE D6} {$DEFINE D5} {$DEFINE D4} {$DEFINE D3} {$DEFINE D2}
          ...
          {$IF CompilerVersion >= 15.00}
            ...
            {$IF CompilerVersion >= 17.00}
              ...
              {$IF CompilerVersion >= 22.00}
                // Delphi 2011 XE and up
                {$DEFINE _D15UP_}
              {$IFEND}
              {$IF CompilerVersion >= 23.00}
                // Delphi 2012 XE2 and up
                {$DEFINE _D16UP_}
              {$IFEND}
            {$IFEND}
...
{$ENDIF}
 ...
 TRegExpr = class
   private
    ...
    function GetExpression : RegExprString; {$IFDEF _INLINE_} inline; {$ENDIF}
    procedure SetExpression (const s : RegExprString); {$IFDEF _INLINE_} inline; {$ENDIF}
    ...
    function GetSubExprMatchCount : integer; {$IFDEF _INLINE_} inline; {$ENDIF}
    function GetMatchPos (Idx : integer) : integer; {$IFDEF _INLINE_} inline; {$ENDIF}
    function GetMatchLen (Idx : integer) : integer; {$IFDEF _INLINE_} inline; {$ENDIF}
    function GetMatch (Idx : integer) : RegExprString; {$IFDEF _INLINE_} inline; {$ENDIF}

    function GetInputString : RegExprString; {$IFDEF _INLINE_} inline; {$ENDIF}
    ...
    procedure SetLineSeparators (const AStr : RegExprString); {$IFDEF _INLINE_} inline; {$ENDIF}
    procedure SetLinePairedSeparator (const AStr : RegExprString);
    function GetLinePairedSeparator : RegExprString; {$IFDEF _INLINE_} inline; {$ENDIF}
   ...
   public
    ...
    // If true, then р-џ  additional include russian letter 'И',
    // Р-п  additional include 'Ј', and р-п include all russian symbols.
...
{$IFNDEF MSWINDOWS}
//(*
function CharUpper(C: PChar): Char;
var
  s: string;
begin
  if assigned(c) then
...
function CharLower(C: PChar): Char;
var
  s: string;
begin
  if assigned(c) then
...
function StrPCopy (Dest: PRegExprChar; const Source: RegExprString): PRegExprChar; {$IFDEF _INLINE_} inline; {$ENDIF}
 var
  i, Len : Integer;
 begin
...
function StrLCopy (Dest, Source: PRegExprChar; MaxLen: Cardinal): PRegExprChar; {$IFDEF _INLINE_} inline; {$ENDIF}
 var i: Integer;
 begin
...
function StrLen (Str: PRegExprChar): Cardinal; {$IFDEF _INLINE_} inline; {$ENDIF}
 begin
  Result:=0;
  while Str [result] <> #0
   do Inc (Result);
...
function StrPos (Str1, Str2: PRegExprChar): PRegExprChar; {$IFDEF _INLINE_} inline; {$ENDIF}
 var n: Integer;
 begin
  Result := nil;
...
function StrLComp (Str1, Str2: PRegExprChar; MaxLen: Cardinal): Integer; {$IFDEF _INLINE_} inline; {$ENDIF}
...
function StrScan (Str: PRegExprChar; Chr: WideChar): PRegExprChar; {$IFDEF _INLINE_} inline; {$ENDIF}
 begin
  Result := nil;
        ...
        if (i < Len) and (ARegExpr [i + 1] = '?') then begin
           ...
           if i > Len
            then Result := -1 // unbalansed '('
             ...
             if TRegExpr.ParseModifiersStr (System.Copy (AnsiString(ARegExpr), i, i - i0), Modif)
...
//      StrPCopy (fExpression, s); //###0.950 replaced due to StrPCopy limitation of 255 chars
      ...
      {$ELSE}
      AnsiStrings.StrLCopy (fExpression, PRegExprChar (s), Len); //###0.950
      {$ENDIF UniCode}
...
// !!! Be carefull - this is class function and must not use object instance fields
  ...
  for i := 1 to length (AModifiers) do
   if AModifiers [i] = '-'
    then IsOn := false
      ...
      if Pos(string(AModifiers [i]), 'iI') > 0
       then Mask := MaskModI
      else if Pos (string(AModifiers [i]), 'rR') > 0
       then Mask := MaskModR
      else if Pos (string(AModifiers [i]), 'sS') > 0
       then Mask := MaskModS
      else if Pos (string(AModifiers [i]), 'gG') > 0
       then Mask := MaskModG
      else if Pos (string(AModifiers [i]), 'mM') > 0
       then Mask := MaskModM
      else if Pos (string(AModifiers [i]), 'xX') > 0
       then Mask := MaskModX
      else begin
        Result := false;
        EXIT;
 ...
 end; {+}{$hints on}{$warnings on}{+.} { of function TRegExpr.ParseModifiersStr
--------------------------------------------------------------}
...
{$IFDEF UniCode}
 ...
 RusRangeLoLow = #$430{'р'};
 RusRangeLoHigh = #$44F{'џ'};
 RusRangeHiLow = #$410{'Р'};
 RusRangeHiHigh = #$42F{'п'};
{$ELSE}
 RusRangeLo = 'рстуфхИцчшщъыьэюя№ёђѓєѕіїјљњћќ§ўџ';
 RusRangeHi = 'РСТУФХЈЦЧШЩЪЫЬЭЮЯабвгдежзийклмноп';
 RusRangeLoLow = 'р';
 RusRangeLoHigh = 'џ';
 RusRangeHiLow = 'Р';
 RusRangeHiHigh = 'п';
  ...
  if PREOp (regnext (scan))^ = EEND then begin // Only one top-level choice.
    ...
    if (flags and SPSTART) <> 0 then begin
        ...
        while scan <> nil do begin
          if (PREOp (scan)^ = EXACTLY)
             and ({$IFDEF UNICODE} strlen {$ELSE} AnsiStrings.StrLen {$ENDIF} (scan + REOpSz + RENextOffSz) >= len) then begin
              longest := scan + REOpSz + RENextOffSz;
              len := {$IFDEF UNICODE} strlen {$ELSE} AnsiStrings.StrLen {$ENDIF} (longest);
           end;
          scan := regnext (scan);
         end;
        regmust := longest;
 ...
 begin
  ...
  case op of
   ...
   '{': begin
      ...
      inc (regparse);
      p := regparse;
      while Pos (string(regparse^), '0123456789') > 0  // <min> MUST appear
       do inc (regparse);
      ...
      if regparse^ = ',' then begin
         inc (regparse);
         p := regparse;
         while Pos (string(regparse^), '0123456789') > 0
          do inc (regparse);
         if regparse^ <> '}' then begin
           regparse := savedparse;
           EXIT;
     ...
     case scan^ of
         ...
         EXACTLYCI: begin
            ...
            if (opnd^ <> reginput^)
             ...
             then EXIT;
            len := {$IFDEF UNICODE} strlen {$ELSE} AnsiStrings.StrLen {$ENDIF} (opnd);
            //###0.929 begin
            no := len;
            save := reginput;
            while no > 1 do begin
...
..............................................................}
 ...
 begin
  ...
  if regmust <> nil then begin
    s := StartPtr;
    REPEAT
     s := {$IFDEF UNICODE} StrScan {$ELSE} AnsiStrings.StrScan {$ENDIF} (s, regmust [0]);
     if s <> nil then begin
       if {$IFDEF UNICODE} StrLComp {$ELSE} AnsiStrings.StrLComp {$ENDIF} (s, regmust, regmlen) = 0
        then BREAK; // Found it.
       inc (s);
      end;
    UNTIL s = nil;
  ...
  if regstart <> #0 then // We know what char it must start with.
    REPEAT
     s := {$IFDEF UNICODE} StrScan {$ELSE} AnsiStrings.StrScan {$ENDIF} (s, regstart);
     if s <> nil then begin
       Result := RegMatch (s);
       if Result
        then EXIT
...
procedure TRegExpr.SetInputString (const AInputString : RegExprString);
 ...
 begin
  ...
  {$ELSE}
  AnsiStrings.StrLCopy (fInputString, PRegExprChar(AInputString), Len);
  //Move(Pointer(PRegExprChar(AInputString))^, Pointer(fInputString)^, Len);
  //fInputString[Len] := #0;
  {$ENDIF}
  ...
  !! startp/endp тёх №ртэю сѓфхђ юярёэю шёяюыќчютрђќ ?
  //*)
...
function TRegExpr.ReplaceEx (AInputStr : RegExprString;
 ...
 begin
  ...
  if Exec (AInputStr) then
   REPEAT
    Result := Result + RegExprString(System.Copy (AInputStr, PrevPos,
      MatchPos [0] - PrevPos))
     + RegExprString(AReplaceFunc (Self));
    PrevPos := MatchPos [0] + MatchLen [0];
   UNTIL not ExecNext;
  Result := Result + System.Copy (AInputStr, PrevPos, MaxInt); // Tail
...
{$ENDIF}
 ...
 begin
  ...
  if AErrorID < 1000 // compilation error ?
   then e := ERegExpr.Create (string(ErrorMsg (AErrorID)) // yes - show error pos
             + ' (pos ' + IntToStr (CompilerErrorPos) + ')')
   else e := ERegExpr.Create (string(ErrorMsg (AErrorID)));
